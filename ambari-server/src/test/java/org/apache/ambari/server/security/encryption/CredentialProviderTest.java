...
public class CredentialProviderTest extends TestCase {
	...
	private static final Log LOG = LogFactory.getLog(CredentialProviderTest.class);
	...
	private void createMasterKey(String dir) {
		File f = new File(dir + System.getProperty("file" + ".separator") + Configuration.MASTER_KEY_FILENAME_DEFAULT);
		if (!f.exists()) {
			try {
				f.createNewFile();
			} catch (IOException e) {
		...
		if (!ms.isMasterKeyInitialized()) { throw new ExceptionInInitializerError("Cannot create master key."); }
	}
