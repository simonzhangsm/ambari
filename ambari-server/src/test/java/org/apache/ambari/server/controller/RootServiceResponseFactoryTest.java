...
public class RootServiceResponseFactoryTest {
	...
	public void testGetRootServiceComponents() throws Exception {
		...
		for (int i = 0; i < RootServiceResponseFactory.Services.AMBARI.getComponents().length; i++) {
			...
			if (component.name().equals(RootServiceResponseFactory.Components.AMBARI_SERVER.name())) assertTrue(rootServiceComponents.contains(new RootServiceComponentResponse(component.name(), ambariMetaInfo.getServerVersion(), configs.getAmbariProperties())));
			else
		...
		assertTrue(rootServiceComponents.contains(new RootServiceComponentResponse(RootServiceResponseFactory.Services.AMBARI.getComponents()[0].name(), ambariMetaInfo.getServerVersion(), configs.getAmbariProperties())));
