...
public class TestStageUtils {
	...
	public static void addService(Cluster cl, List<String> hostList, Map<String, List<Integer>> topology, String serviceName, Injector injector) throws AmbariException {
	...
	private void checkServiceCompression(Map<String, Set<String>> info, Map<String, String> serviceMapping, Map<String, List<Integer>> serviceTopology, List<String> hostList) {
	...
	private List<Integer> getRangeMappedDecompressedSet(Set<String> compressedSet) {
		...
		for (String token : compressedSet) {
			...
			if (split.length != 2) throw new RuntimeException("Broken data, expected format - m:r, got - " + token);
	...
	private List<Integer> getReindexedList(List<Integer> list, List<String> currentIndexes, List<String> desiredIndexes) {
