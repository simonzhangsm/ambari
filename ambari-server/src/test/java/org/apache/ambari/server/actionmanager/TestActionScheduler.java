...
public class TestActionScheduler {
	...
	private List<AgentCommand> waitForQueueSize(String hostname, ActionQueue aq, int expectedQueueSize) throws InterruptedException {
		while (true) {
	...
	public void testActionTimeout() throws Exception {
		...
		// Small action timeout to test rescheduling
		ActionScheduler scheduler = new ActionScheduler(100, 50, db, aq, fsm, 3, new HostsMap((String) null), null, unitOfWork, conf);
		scheduler.setTaskTimeoutAdjustment(false);
		// Start the thread
		scheduler.start();
	...
	public void testOpFailedEventRaisedForAbortedHostRole() throws Exception {
		...
		Stage stage = new Stage(1, "/tmp", "cluster1", "stageWith2Tasks", CLUSTER_HOST_INFO);
		...
		while (!stages.get(0).getHostRoleStatus(hostname1, "DATANODE").equals(HostRoleStatus.TIMEDOUT) && !stages.get(0).getHostRoleStatus(hostname2, "NAMENODE").equals(HostRoleStatus.ABORTED)) {
	...
	private static Stage getStageWithServerAction(long requestId, long stageId, String hostName, Map<String, String> payload, String requestContext) {
		Stage stage = new Stage(requestId, "/tmp", "cluster1", requestContext, CLUSTER_HOST_INFO);
		stage.setStageId(stageId);
		long now = System.currentTimeMillis();
		stage.addServerActionCommand(ServerAction.Command.FINALIZE_UPGRADE, Role.AMBARI_SERVER_ACTION, RoleCommand.EXECUTE, "cluster1", new ServiceComponentHostUpgradeEvent("AMBARI_SERVER_ACTION", hostName, now, "HDP-0.2"), hostName);
		ExecutionCommand execCmd = stage.getExecutionCommandWrapper(hostName, Role.AMBARI_SERVER_ACTION.toString()).getExecutionCommand();
	...
	/**
	 * Verifies that stages that are executed on different hosts and rely to different requests are scheduled to be executed in parallel
	 */
	@Test
	public void testIndependentStagesExecution() throws Exception {
		...
		getStageWithSingleTask(hostname1, "cluster1", Role.GANGLIA_MONITOR, RoleCommand.START, Service.Type.GANGLIA, 2, 2, 2));
		...
		getStageWithSingleTask(hostname4, "cluster1", Role.GANGLIA_MONITOR, RoleCommand.START, Service.Type.GANGLIA, 5, 5, 4));
	...
	public void testRequestFailureOnStageFailure() throws Exception {
		...
		when(db.getTask(anyLong())).thenAnswer(new Answer<Object>() {
			...
			public Object answer(InvocationOnMock invocation) throws Throwable {
				...
				for (Stage stage : stages) {
					for (HostRoleCommand command : stage.getOrderedHostRoleCommands()) {
						if (taskId.equals(command.getTaskId())) { return command; }
					}
				}
				return null;
			}
	...
	/**
	 * Tests that the whole request is aborted when there are no QUEUED tasks for a role and success factor is not met. As long as there is one QUEUED task the request is not aborted.
	 * 
	 * @throws Exception
	 */
	@Test
	public void testRequestAbortsOnlyWhenNoQueuedTaskAndSuccessFactorUnmet() throws Exception {
		...
		Properties properties = new Properties();
		Configuration conf = new Configuration(properties);
		ActionScheduler scheduler = new ActionScheduler(100, 10000, db, aq, fsm, 3, new HostsMap((String) null), new ServerActionManagerImpl(fsm), unitOfWork, conf);
		ActionManager am = new ActionManager(2, 10000, aq, fsm, db, new HostsMap((String) null), new ServerActionManagerImpl(fsm), unitOfWork, requestFactory, conf);
	...
	private void addHostRoleExecutionCommand(long now, Stage stage, Role role, Service.Type service, RoleCommand command, String host, String cluster) {
		stage.addHostRoleExecutionCommand(host, role, command, new ServiceComponentHostInstallEvent(role.toString(), host, now, "HDP-0.2"), cluster, service.toString());
	...
	private CommandReport getCommandReport(HostRoleStatus status, Role role, Service.Type service, String actionId, int taskId) {
	...
	private Stage getStageWithSingleTask(String hostname, String clusterName, Role role, RoleCommand roleCommand, Service.Type service, int taskId, int stageId, int requestId) {
		Stage stage = new Stage(requestId, "/tmp", clusterName, "getStageWithSingleTask", CLUSTER_HOST_INFO);
		stage.setStageId(stageId);
		stage.addHostRoleExecutionCommand(hostname, role, roleCommand, new ServiceComponentHostUpgradeEvent(role.toString(), hostname, System.currentTimeMillis(), "HDP-0.2"), clusterName, service.toString());
	...
	private void addInstallTaskToStage(Stage stage, String hostname, String clusterName, Role role, RoleCommand roleCommand, Service.Type service, int taskId) {
		
		stage.addHostRoleExecutionCommand(hostname, role, roleCommand, new ServiceComponentHostInstallEvent(role.toString(), hostname, System.currentTimeMillis(), "HDP-0.2"), clusterName, service.toString());
	...
	/**
	 * Checks what happens when stage has an execution command for host component that has been recently deleted
	 * 
	 * @throws Exception
	 */
	@Test
