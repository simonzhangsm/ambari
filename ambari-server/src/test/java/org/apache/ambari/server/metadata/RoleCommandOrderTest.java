...
public class RoleCommandOrderTest {
	...
	/**
	 * Tests building dependencies in GLUSTERFS cluster. Uses real dependency mapping file (role_command_order.json)
	 */
	@Test
	...
	/**
	 * Tests building dependencies in not HA-enabled HDFS cluster. Uses real dependency mapping file (role_command_order.json)
	 */
	@Test
	...
	/**
	 * Tests building dependencies in HA-enabled HDFS cluster. Uses real dependency mapping file (role_command_order.json)
	 */
	@Test
	...
	public void testAddDependencies() throws IOException {
		...
		String expected = "{\"RoleCommandPair{role=SECONDARY_NAMENODE, " + "cmd=UPGRADE}\":[{\"role\":{\"name\":\"NAMENODE\"},\"cmd\":\"UPGRADE\"}]," + "\"RoleCommandPair{role=SECONDARY_NAMENODE, cmd=START}\":[{\"role\":{\"name\":\"NAMENODE\"},"
				+ "\"cmd\":\"START\"}],\"RoleCommandPair{role=DATANODE, cmd=STOP}\":[{\"role\":" + "{\"name\":\"HBASE_MASTER\"},\"cmd\":\"STOP\"},{\"role\":{\"name\":\"RESOURCEMANAGER\"}," + "\"cmd\":\"STOP\"},{\"role\":{\"name\":\"TASKTRACKER\"},\"cmd\":\"STOP\"},"
				+ "{\"role\":{\"name\":\"NODEMANAGER\"},\"cmd\":\"STOP\"},{\"role\":{\"name\":\"HISTORYSERVER\"}," + "\"cmd\":\"STOP\"},{\"role\":{\"name\":\"JOBTRACKER\"},\"cmd\":\"STOP\"}]}";
	...
	private boolean dependenciesContainBlockedRole(Map<RoleCommandPair, Set<RoleCommandPair>> deps, Role blocked) {
		for (RoleCommandPair blockedPair : deps.keySet()) {
			if (blockedPair.getRole() == blocked) { return true; }
		}
		return false;
	}
	
	private boolean dependenciesContainBlockerRole(Map<RoleCommandPair, Set<RoleCommandPair>> deps, Role blocker) {
		for (Set<RoleCommandPair> blockerSet : deps.values()) {
			for (RoleCommandPair roleCommandPair : blockerSet) {
				if (roleCommandPair.getRole() == blocker) { return true; }
			}
		}
		return false;
	}
