...
public class CredentialStoreServiceTest extends TestCase {
	...
	private static final Log LOG = LogFactory.getLog(CredentialStoreServiceTest.class);
	...
	protected void setUp() throws Exception {
		...
		credentialStoreService = new CredentialStoreServiceImpl(keystore_dir.getAbsolutePath());
	}
	...
	public void testAddCredentialToStoreWithPersistMaster() throws Exception {
		...
		MasterKeyService masterKeyService = new MasterKeyServiceImpl(masterKey, masterKeyLocation, true);
	...
	public void testAliasParsing() throws Exception {
		...
		Pattern PASSWORD_ALIAS_PATTERN = Pattern.compile("\\$\\{alias=[\\w\\.]+\\}");
		Matcher matcher = PASSWORD_ALIAS_PATTERN.matcher(strPasswd);
		Assert.assertTrue(matcher.matches());
		Assert.assertEquals("ambari.password", strPasswd.substring(strPasswd.indexOf("=") + 1, strPasswd.length() - 1));
	}
