...
public class TestHeartbeatMonitor {
	...
	public void testStateCommandsGeneration() throws AmbariException, InterruptedException, InvalidStateTransitionException {
		...
		Config config = configFactory.createNew(cluster, "global", new HashMap<String, String>() {
			{
				put("a", "b");
			}
		});
	...
	public void testHeartbeatLossWithComponent() throws AmbariException, InterruptedException, InvalidStateTransitionException {
		...
		for (ServiceComponentHost sch : cluster.getServiceComponentHosts(hostname1)) {
			...
			if (sch.getServiceComponentName().equals("NAMENODE")) assertEquals(sch.getServiceComponentName(), State.UNKNOWN, sch.getState());
			else if (sch.getServiceComponentName().equals("DATANODE")) assertEquals(sch.getServiceComponentName(), State.INSTALLING, sch.getState());
			else if (sc.isClientComponent()) assertEquals(sch.getServiceComponentName(), State.INIT, sch.getState());
			else if (sch.getServiceComponentName().equals("SECONDARY_NAMENODE")) assertEquals(sch.getServiceComponentName(), State.DISABLED, sch.getState());
		}
	}
}
