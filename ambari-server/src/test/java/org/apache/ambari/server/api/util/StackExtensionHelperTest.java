...
public class StackExtensionHelperTest {
	...
	private final String stackRootStr = "./src/test/resources/stacks/".replaceAll("/", File.separator);
	private final String hBaseDirStr = stackRootStr + "services/HBASE/".replaceAll("/", File.separator);
	private final String yarnDirStr = stackRootStr + "services/YARN/".replaceAll("/", File.separator);
	
	/**
	 * Checks than service metainfo is parsed correctly both for ver 1 services and for ver 2 services
	 */
	@Test
	...
	public void getSchemaVersion() throws Exception {
		File stackRoot = new File(stackRootStr);
		StackExtensionHelper helper = new StackExtensionHelper(stackRoot);
		File legacyMetaInfoFile = new File("./src/test/resources/stacks/HDP/2.0.7/" + "services/HIVE/metainfo.xml".replaceAll("/", File.separator));
		String version = helper.getSchemaVersion(legacyMetaInfoFile);
		assertEquals("1.0", version);
		
		File v2MetaInfoFile = new File("./src/test/resources/stacks/HDP/2.0.7/" + "services/HBASE/metainfo.xml".replaceAll("/", File.separator));
