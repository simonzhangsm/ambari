...
public class AmbariMetaInfoTest {
	...
	public void testConfigDependencies() throws Exception {
		ServiceInfo serviceInfo = metaInfo.getServiceInfo(STACK_NAME_HDP, EXT_STACK_NAME, SERVICE_NAME_MAPRED2);
	...
	public void testGlobalMapping() throws Exception {
		...
		for (PropertyInfo pinfol : pinfo) {
			if ("global.xml".equals(pinfol.getFilename())) {
				if ("hadoop_heapsize".equals(pinfol.getName())) checkforhadoopheapsize = true;
			}
		}
	...
	public void testServicePackageDirInheritance() throws Exception {
		...
		Assert.assertEquals("HDP/2.0.7/services/HBASE/package", service.getServicePackageFolder());
		
		service = metaInfo.getService(STACK_NAME_HDP, "2.0.7", "HDFS");
		Assert.assertEquals("HDP/2.0.7/services/HDFS/package", service.getServicePackageFolder());
		...
		Assert.assertEquals("HDP/2.0.8/services/HDFS/package", service.getServicePackageFolder());
	}
	...
	private CustomCommandDefinition findCustomCommand(String ccName, ServiceInfo service) {
		for (CustomCommandDefinition ccd : service.getCustomCommands()) {
			if (ccd.getName().equals(ccName)) { return ccd; }
		}
		return null;
	}
	
	private CustomCommandDefinition findCustomCommand(String ccName, ComponentInfo component) {
	...
	public void testPasswordPropertyAttribute() throws Exception {
		...
		for (PropertyInfo propertyInfo : propertyInfoList) {
			if (propertyInfo.isRequireInput() && propertyInfo.getType().equals(PropertyInfo.PropertyType.PASSWORD)) {
