...
public class TestIvoryService implements IvoryService {
	...
	public TestIvoryService(Map<String, Feed> feeds, Map<String, Cluster> clusters, HashMap<String, Map<String, Instance>> instanceMap) {
		if (feeds != null) {
	...
	public List<Instance> getInstances(String feedName) {
		...
		if (instances != null) { return new LinkedList<Instance>(instances.values()); }
	...
	private String setFeedStatus(String feedName, String status) {
		...
		if (feed != null) {
			...
			if (!currentStatus.equals(status)) {
				feed = new Feed(feed.getName(), feed.getDescription(), status, feed.getSchedule(), feed.getSourceClusterName(), feed.getSourceClusterStart(), feed.getSourceClusterEnd(), feed.getSourceClusterLimit(), feed.getSourceClusterAction(), feed.getTargetClusterName(),
						feed.getTargetClusterStart(), feed.getTargetClusterEnd(), feed.getTargetClusterLimit(), feed.getTargetClusterAction(), feed.getProperties());
