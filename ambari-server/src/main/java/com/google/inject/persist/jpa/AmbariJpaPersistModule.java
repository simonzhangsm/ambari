...
public class AmbariJpaPersistModule extends PersistModule {
	 ...
	 * 
	 * @param properties A set of name value pairs that configure a JPA persistence provider as per the specification.
	 */
	...
	private <T> void bindFinder(Class<T> iface) {
		if (!isDynamicFinderValid(iface)) { return; }
		...
		T proxy = (T) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), new Class<?>[] { iface }, finderInvoker);
