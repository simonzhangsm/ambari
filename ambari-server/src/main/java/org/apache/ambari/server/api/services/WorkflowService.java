...
public class WorkflowService extends BaseService {
	...
	@Path("{workflowId}")
	@Produces("text/plain")
	public Response getWorkflow(@Context HttpHeaders headers, @Context UriInfo ui, @PathParam("workflowId") String workflowId) {
		return handleRequest(headers, null, ui, Request.Type.GET, createWorkflowResource(clusterName, workflowId));
	}
	...
	public Response getWorkflows(@Context HttpHeaders headers, @Context UriInfo ui) {
		return handleRequest(headers, null, ui, Request.Type.GET, createWorkflowResource(clusterName, null));
	}
