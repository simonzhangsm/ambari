...
/**
 * Minimal Renderer. All href properties are stripped from the result. For the root resource, this renderer behaves identically to the default renderer wrt resource properties and sub-resources. If no root properties or sub-resources are specified all top level properties and all
 * sub-resources are included in the result. If any root properties or any sub-resources are requested, then only those will be included in the result. For sub-resource, only primary keys and any requested properties are included in the result. This renderer can be specified for
 * any resource using 'format=minimal' or the older syntax 'minimal_response=true'.
 */
public class MinimalRenderer extends BaseRenderer implements Renderer {
	...
	/**
	 * Recursively strip all unwanted properties from the result nodes. During normal processing, foreign keys are always added to the request which need to be stripped unless they were requested.
	 * 
	...
	/**
	 * Checks if properties contains a category for the subCategoryName Example: metrics/yarn/Queue/root/ActiveApplications and metrics/yarn/Queue
	 * 
	...
	private boolean isSubCategory(Set<String> properties, String subCategoryName) {
		for (String property : properties) {
			if (subCategoryName.startsWith(property)) { return true; }
		}
		return false;
	}
	
	/**
	 * Obtain the primary keys for the specified type. This method is necessary because some resource types, specifically the configuration type, don't have a proper pk even though one is registered. Instead, multiple properties are used as a 'composite' key even though this is
