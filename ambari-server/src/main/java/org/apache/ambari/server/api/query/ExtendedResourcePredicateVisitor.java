...
public class ExtendedResourcePredicateVisitor implements PredicateVisitor {
	...
	/**
	 * Predicate implementation used to replace any existing predicate and extend the resource being evaluated with an extended set of property values.
	 */
	private static class ExtendedResourcePredicate implements Predicate {
		...
		public boolean evaluate(Resource resource) {
			...
			if (extendedPropertySet == null) { return predicate.evaluate(resource); }
			...
			for (Map<String, Object> extendedProperties : extendedPropertySet) {
				...
				if (predicate.evaluate(extendedResource)) { return true; }
			}
			return false;
		}
	}
	...
	/**
	 * A resource that extends a given resource by copying it and adding additional properties.
	 */
