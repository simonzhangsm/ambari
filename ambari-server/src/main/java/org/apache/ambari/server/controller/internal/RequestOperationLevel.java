...
/**
 * Operation level is specified along with some requests. It identifies the logical level, at which the operation is executed.
 */
public class RequestOperationLevel {
	
	/**
	 * Conversion table is used to convert user input into our internal names of resources, defined at org.apache.ambari.server.controller.spi.Resource.Type
	 */
	private static final String[][] LEVEL_ALIASES = new String[][] {
			// FORMAT: <external alias> , <internal alias>
			{ "CLUSTER", "Cluster" }, { "SERVICE", "Service" }, { "HOST", "Host" }, { "HOST_COMPONENT", "HostComponent" }, };
	...
	public static String getInternalLevelName(String external) throws IllegalArgumentException {
		...
		for (String[] pair : LEVEL_ALIASES) {
			if (pair[ALIAS_COLUMN].equals(refinedAlias)) { return pair[INTERNAL_NAME_COLUMN]; }
		}
	...
	public static String getExternalLevelName(String internal) {
		for (String[] pair : LEVEL_ALIASES) {
			if (pair[INTERNAL_NAME_COLUMN].equals(internal)) { return pair[ALIAS_COLUMN]; }
		}
	...
	/**
	 * Valid values are Cluster, Service, Host and HostComponent. Component level is identical to Service level, and that's why it is not supported as a standalone level.
	 */
	...
	/**
	 * Source service for request. Specified for Service-level and HostComponent-level requests.
	 */
	private String serviceName;
	
	/**
	 * Source host component for request. Specified for HostComponent-level requests.
	 */
	private String hostComponentName;
	
	/**
	 * Source host for request. Specified for Host-level and HostComponent-level requests.
	 */
