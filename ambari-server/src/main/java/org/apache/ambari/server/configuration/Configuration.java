...
/**
 * Ambari configuration. Reads properties from ambari.properties
 */
@Singleton
public class Configuration {
	...
	public static final String BOOTSTRAP_SETUP_AGENT_SCRIPT_DEFAULT = "/usr/lib/python3.4/site-packages/ambari_server/setupAgent.py";
	...
	/**
	 * This key defines whether stages of parallel requests are executed in parallel or sequentally. Only stages from different requests running on not interfering host sets may be executed in parallel.
	 */
	 ...
	 * 
	 * @param properties properties to use for testing and in production using the Conf object.
	 */
	...
	protected void loadSSLParams() {
		...
		if (properties.getProperty(SSL_TRUSTSTORE_PASSWORD_KEY) != null) {
			String ts_password = readPasswordFromStore(properties.getProperty(SSL_TRUSTSTORE_PASSWORD_KEY));
			if (ts_password != null) {
				System.setProperty(JAVAX_SSL_TRUSTSTORE_PASSWORD, ts_password);
			} else {
				System.setProperty(JAVAX_SSL_TRUSTSTORE_PASSWORD, properties.getProperty(SSL_TRUSTSTORE_PASSWORD_KEY));
			}
		}
	...
	private synchronized void loadCredentialProvider() {
		if (credentialProviderInitialized) {
			...
			try {
				this.credentialProvider = new CredentialProvider(null, getMasterKeyLocation(), isMasterKeyPersisted());
			} catch (Exception e) {
	...
	private static Properties readConfigFile() {
		...
		if (inputStream == null) throw new RuntimeException(CONFIG_FILE + " not found in classpath");
	...
	public String getBootSetupAgentScript() {
		return properties.getProperty(BOOTSTRAP_SETUP_AGENT_SCRIPT, BOOTSTRAP_SETUP_AGENT_SCRIPT_DEFAULT);
	}
	
	public String getBootSetupAgentPassword() {
		String pass = configsMap.get(PASSPHRASE_KEY);
		
		if (null != pass) return pass;
	...
	/**
	 * Get the map with server config parameters. Keys - public constants of this class
	 * 
	...
	public boolean getTwoWaySsl() {
		return ("true".equals(properties.getProperty(SRVR_TWO_WAY_SSL_KEY, SRVR_TWO_WAY_SSL_DEFAULT)));
	}
	
	/**
	 * Check persistence type Ambari Server should use. Possible values: in-memory - use in-memory Derby database to store data local - use local Postgres instance remote - use provided jdbc driver name and url to connect to database
	 */
	...
	public String getLocalDatabaseUrl() {
		...
		if (dbName == null || dbName.isEmpty()) throw new RuntimeException("Server DB Name is not configured!");
	...
	public String getDatabasePassword() {
		...
		if (dbpasswd != null) return dbpasswd;
		else
	...
	public LdapServerProperties getLdapServerProperties() {
		...
		String ldapPasswd = readPasswordFromStore(properties.getProperty(LDAP_MANAGER_PASSWORD_KEY));
		if (ldapPasswd != null) {
		...
		if (properties.containsKey(LDAP_GROUP_BASE_KEY) || properties.containsKey(LDAP_GROUP_OBJECT_CLASS_KEY) || properties.containsKey(LDAP_GROUP_MEMEBERSHIP_ATTR_KEY) || properties.containsKey(LDAP_GROUP_NAMING_ATTR_KEY)
				|| properties.containsKey(LDAP_ADMIN_GROUP_MAPPING_RULES_KEY) || properties.containsKey(LDAP_GROUP_SEARCH_FILTER_KEY)) {
	...
	/**
	 * @return the name to be used for audit information if there is no logged-in user. Default is '_anonymous'.
	 */
	...
	public String getMasterKeyLocation() {
		String defaultDir = properties.getProperty(MASTER_KEY_LOCATION, properties.getProperty(SRVR_KSTR_DIR_KEY, SRVR_KSTR_DIR_DEFAULT));
	...
	public Map<String, String> getDatabaseCustomProperties() {
		if (null != customDbProperties) { return customDbProperties; }
	...
	public String[] getRepoValidationSuffixes(String osFamily) {
		...
		if (osFamily.equals(UBUNTU_OS)) {
			repoSuffixes = properties.getProperty(REPO_SUFFIX_KEY_UBUNTU, REPO_SUFFIX_UBUNTU);
		} else {
	...
	public String getExecutionSchedulerThreads() {
		return properties.getProperty(EXECUTION_SCHEDULER_THREADS, DEFAULT_SCHEDULER_THREAD_COUNT);
	}
	...
	public Long getExecutionSchedulerMisfireToleration() {
		String limit = properties.getProperty(EXECUTION_SCHEDULER_MISFIRE_TOLERATION, DEFAULT_EXECUTION_SCHEDULER_MISFIRE_TOLERATION);
		return Long.parseLong(limit);
	}
	
	public Integer getExecutionSchedulerStartDelay() {
		String delay = properties.getProperty(EXECUTION_SCHEDULER_START_DELAY, DEFAULT_SCHEDULER_START_DELAY_SECONDS);
	...
	public String getCustomActionDefinitionPath() {
		return properties.getProperty(CUSTOM_ACTION_DEFINITION_KEY, CUSTOM_ACTION_DEFINITION_DEF_VALUE);
	}
	
	/**
	 * @return default task timeout in seconds (string representation). This value is used at python (agent) code.
	 */
	public String getDefaultAgentTaskTimeout() {
		...
		if (StringUtils.isNumeric(value)) {
			...
			LOG.warn(String.format("Value of %s (%s) should be a number, " + "falling back to default value (%s)", AGENT_TASK_TIMEOUT_KEY, value, AGENT_TASK_TIMEOUT_DEFAULT));
	...
	public int getClientThreadPoolSize() {
		return Integer.parseInt(properties.getProperty(CLIENT_THREADPOOL_SIZE_KEY, String.valueOf(CLIENT_THREADPOOL_SIZE_DEFAULT)));
	}
	...
	public int getAgentThreadPoolSize() {
		return Integer.parseInt(properties.getProperty(AGENT_THREADPOOL_SIZE_KEY, String.valueOf(AGENT_THREADPOOL_SIZE_DEFAULT)));
	}
}
