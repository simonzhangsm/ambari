...
public abstract class AbstractPropertyProvider extends BaseProvider implements PropertyProvider {
	...
	/**
	 * Regular expression for checking a property id for a metric argument with methods. (e.g. metrics/yarn/Queue$1.replaceAll(\",q(\\d+)=\",\"/\")/AppsRunning)
	 */
	private static final Pattern CHECK_FOR_METRIC_ARGUMENT_METHODS_REGEX = Pattern.compile("(\\$\\d\\.[^\\$]+\\))+");
	
	/**
	 * Regular expression for extracting the methods from a metric argument. (e.g. $1.replaceAll(\",q(\\d+)=\",\"/\"))
	 */
	private static final Pattern FIND_ARGUMENT_METHOD_REGEX = Pattern.compile(".\\w+\\(.*?\\)");
	
	/**
	 * Regular expression for extracting the arguments for methods from a metric argument. Only strings and integers are supported.
	 */
	...
	/**
	 * Get a map of metric / property info based on the given component name and property id. Note that the property id may map to multiple metrics if the property id is a category.
	 * 
	...
	protected void getPropertyInfoMap(String componentName, String propertyId, Map<String, PropertyInfo> propertyInfoMap) {
		...
		if (componentMetricMap == null) { return; }
	...
	/**
	 * Substitute the given value into the argument in the given property id. If there are methods attached to the argument then execute them for the given value.
	 * 
	...
	protected void updateComponentMetricMap(Map<String, PropertyInfo> componentMetricMap, String propertyId) {
		...
		if (!componentMetricMap.containsKey(propertyId) && regExpKey != null && !regExpKey.equals(propertyId)) {
			...
			if (propertyInfo != null) {
				...
				componentMetricMap.put(propertyId, new PropertyInfo(key, propertyInfo.isTemporal(), propertyInfo.isPointInTime()));
			}
