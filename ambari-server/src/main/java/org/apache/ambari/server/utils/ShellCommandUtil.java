...
public class ShellCommandUtil {
	...
	/*
	 * public static String LogAndReturnOpenSslExitCode(String command, int exitCode) { logOpenSslExitCode(command, exitCode); return getOpenSslCommandResult(command, exitCode); }
	 */
	...
	public static String getOpenSslCommandResult(String command, int exitCode) {
		return new StringBuilder().append("Command ").append(hideOpenSslPassword(command)).append(" was finished with exit code: ").append(exitCode).append(" - ").append(getOpenSslExitCodeDescription(exitCode)).toString();
	}
	...
	/**
	 * Permission mask 600 allows only owner to read and modify file. Other users (except root) can't read/modify file
	 */
	...
	/**
	 * Gets file permissions on Linux systems. Under Windows/Mac, command always returns MASK_EVERYBODY_RWX
	 * 
	 * @param path
	 */
	...
	/**
	 * Sets file permissions to a given value on Linux systems. On Windows/Mac, command is silently ignored
	 * 
	 * @param mode
	 * @param path
	 */
