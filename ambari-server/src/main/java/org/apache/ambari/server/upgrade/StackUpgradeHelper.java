...
public class StackUpgradeHelper {
	...
	public void updateStackVersion(Map<String, String> stackInfo) throws Exception {
		if (stackInfo == null || stackInfo.isEmpty()) { throw new IllegalArgumentException("Empty stack id. " + stackInfo); }
	...
	public static void main(String[] args) {
		try {
			if (args.length < 2) { throw new InputMismatchException("Need to provide action, " + "stack name and stack version."); }
			...
			if (!stackUpgradeHelper.getValidActions().contains(action)) { throw new IllegalArgumentException("Unsupported action. Allowed " + "actions: " + stackUpgradeHelper.getValidActions()); }
