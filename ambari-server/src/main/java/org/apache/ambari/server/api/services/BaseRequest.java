...
public abstract class BaseRequest implements Request {
	...
	/**
	 * Associated resource renderer. Will default to the default renderer if non is specified.
	 */
	...
	public Result process() {
		...
		try {
			...
			result = new ResultImpl(new ResultStatus(ResultStatus.STATUS.BAD_REQUEST, "Unable to compile query predicate: " + e.getMessage()));
	...
	public PageRequest getPageRequest() {
		...
		if (pageSize == null && from == null && to == null) { return null; }
	...
	/**
	 * Parse the query string and compile it into a predicate. The query string may have already been extracted from the http body. If the query string didn't exist in the body use the query string in the URL.
	 * 
	...
	/**
	 * Parse the query string for the {@link QueryLexer#QUERY_FORMAT} property and obtain a renderer from the associated resource definition based on this property value.
	 */
	private void parseRenderer() {
		String rendererName = isMinimal() ? "minimal" : m_uriInfo.getQueryParameters().getFirst(QueryLexer.QUERY_FORMAT);
