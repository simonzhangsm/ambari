...
@NamedQueries({ @NamedQuery(name = "groupsByHost", query = "SELECT confighosts FROM ConfigGroupHostMappingEntity confighosts " + "WHERE confighosts.hostname=:hostname"),
		@NamedQuery(name = "hostsByGroup", query = "SELECT confighosts FROM ConfigGroupHostMappingEntity confighosts " + "WHERE confighosts.configGroupId=:groupId") })
public class ConfigGroupHostMappingEntity {
	...
	@ManyToOne
	@JoinColumns({ @JoinColumn(name = "host_name", referencedColumnName = "host_name", nullable = false, insertable = false, updatable = false) })
	private HostEntity hostEntity;
	
	@ManyToOne
	@JoinColumns({ @JoinColumn(name = "config_group_id", referencedColumnName = "group_id", nullable = false, insertable = false, updatable = false) })
