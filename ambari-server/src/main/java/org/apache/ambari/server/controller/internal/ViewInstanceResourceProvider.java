...
public class ViewInstanceResourceProvider extends AbstractResourceProvider {
	...
	private Resource toResource(ViewInstanceEntity viewInstanceEntity, Set<String> requestedIds) {
		...
		setResourceProperty(resource, PROPERTIES_PROPERTY_ID, properties, requestedIds);
		...
		setResourceProperty(resource, CONTEXT_PATH_PROPERTY_ID, ViewInstanceEntity.getContextPath(viewName, version, name), requestedIds);
	...
	private ViewInstanceEntity toEntity(Map<String, Object> properties) {
		...
		if (name == null || name.isEmpty()) { throw new IllegalArgumentException("View instance name must be provided"); }
		
		String version = (String) properties.get(VIEW_VERSION_PROPERTY_ID);
		if (version == null || version.isEmpty()) { throw new IllegalArgumentException("View version must be provided"); }
		
		String viewName = (String) properties.get(VIEW_NAME_PROPERTY_ID);
		if (viewName == null || viewName.isEmpty()) { throw new IllegalArgumentException("View name must be provided"); }
