	...
	public Set<RootServiceComponentResponse> getRootServiceComponents(RootServiceComponentRequest request) throws ObjectNotFoundException {
		...
		if (componentName != null) {
			...
			for (Components component : service.getComponents())
				response.add(new RootServiceComponentResponse(component.toString(), getComponentVersion(component.name(), null), getComponentProperties(component.name())));
		}
		return response;
	}
	...
	public Set<RootServiceHostComponentResponse> getRootServiceHostComponent(RootServiceHostComponentRequest request, Set<HostResponse> hosts) throws AmbariException {
		...
		for (RootServiceComponentResponse component : rootServiceComponents) {
			...
			for (HostResponse host : filteredHosts) {
				...
				if (component.getComponentName().equals(Components.AMBARI_SERVER.name())) response.add(new RootServiceHostComponentResponse(host.getHostname(), component.getComponentName(), RUNNING_STATE, getComponentVersion(component.getComponentName(), host), component
						.getProperties()));
				else
					response.add(new RootServiceHostComponentResponse(host.getHostname(), component.getComponentName(), host.getHostState(), getComponentVersion(component.getComponentName(), host), component.getProperties()));
			}
		}
