...
public class OperatingSystemResourceProvider extends ReadOnlyResourceProvider {
	...
	public static final String STACK_NAME_PROPERTY_ID = PropertyHelper.getPropertyId("OperatingSystems", "stack_name");
	
	private static final String STACK_VERSION_PROPERTY_ID = PropertyHelper.getPropertyId("OperatingSystems", "stack_version");
	
	private static final String OS_TYPE_PROPERTY_ID = PropertyHelper.getPropertyId("OperatingSystems", "os_type");
	
	private static Set<String> pkPropertyIds = new HashSet<String>(Arrays.asList(new String[] { STACK_NAME_PROPERTY_ID, STACK_VERSION_PROPERTY_ID, OS_TYPE_PROPERTY_ID }));
	
	protected OperatingSystemResourceProvider(Set<String> propertyIds, Map<Type, String> keyPropertyIds, AmbariManagementController managementController) {
	...
	public Set<Resource> getResources(Request request, Predicate predicate) throws SystemException, UnsupportedPropertyException, NoSuchResourceException, NoSuchParentResourceException {
		...
		for (OperatingSystemResponse response : responses) {
			...
			setResourceProperty(resource, OS_TYPE_PROPERTY_ID, response.getOsType(), requestedIds);
	...
	private OperatingSystemRequest getRequest(Map<String, Object> properties) {
		return new OperatingSystemRequest((String) properties.get(STACK_NAME_PROPERTY_ID), (String) properties.get(STACK_VERSION_PROPERTY_ID), (String) properties.get(OS_TYPE_PROPERTY_ID));
	}
