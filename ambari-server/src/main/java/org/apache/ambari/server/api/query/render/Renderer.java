...
/**
 * Responsible for the rendering of a result. This includes both the content (which properties), and the format of the query result. Format doesn't refer to json or xml, but instead to the structure of the categories, sub-resources and properties. Renderer's are registered for a
 * resource type by adding them to the corresponding resource definition.
 */
public interface Renderer {
	...
	/**
	 * Finalize which properties are requested by the query. This is called once per user query regardless of how many sub-queries the original query is decomposed into.
	 * 
	 * @param queryProperties tree of query information. Contains query information for the root query and all sub-queries (children)
	...
	/**
	 * Obtain the associated post processor. Post Processors existed prior to renderer's to allow the native result to be augmented before returning it to the user. This functionality should be merged into the renderer.
	 * 
