...
public class RoleGraph {
		...
		for (String host : hostRoleCommands.keySet()) {
			for (String role : hostRoleCommands.get(host).keySet()) {
				...
				if (graph.get(role) == null) {
					rgn = new RoleGraphNode(hostRoleCommand.getRole(), hostRoleCommand.getRoleCommand());
					graph.put(role, rgn);
				}
				rgn = graph.get(role);
				rgn.addHost(host);
	...
	/**
	 * Returns a list of stages that need to be executed one after another to execute the DAG generated in the last {@link #build(Stage)} call.
	 */
	...
	private Stage getStageFromGraphNodes(Stage origStage, List<RoleGraphNode> stageGraphNodes) {
		...
		Stage newStage = new Stage(origStage.getRequestId(), origStage.getLogDir(), origStage.getClusterName(), origStage.getRequestContext(), origStage.getClusterHostInfo());
