...
/**
 * Ambari security. Manages server and agent certificates
 */
@Singleton
public class CertificateManager {
	...
	private static final String SIGN_SRVR_CRT = "openssl ca -create_serial " + "-out {1}/{3} -days 365 -keyfile {1}/{2} -key {0} -selfsign " + "-extensions jdk7_ca -config {1}/ca.conf -batch " + "-infiles {1}/{5}";
	...
	private static final String REVOKE_AGENT_CRT = "openssl ca " + "-config {0}/ca.conf -keyfile {0}/{4} -revoke {0}/{2} -batch " + "-passin pass:{3} -cert {0}/{5}";
	private static final String SIGN_AGENT_CRT = "openssl ca -config " + "{0}/ca.conf -in {0}/{1} -out {0}/{2} -batch -passin pass:{3} " + "-keyfile {0}/{4} -cert {0}/{5}";
