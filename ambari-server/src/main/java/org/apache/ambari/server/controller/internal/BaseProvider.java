...
public abstract class BaseProvider {
	...
	protected Set<String> checkConfigPropertyIds(Set<String> base, String configCategory) {
		...
		if (0 == base.size()) return base;
		
		Set<String> unsupported = new HashSet<String>();
		
		for (String propertyId : base) {
			if (!propertyId.startsWith(configCategory + "/desired_config")) unsupported.add(propertyId);
		}
	...
	protected Set<String> getRequestPropertyIds(Request request, Predicate predicate) {
		...
		if (propertyIds == null || propertyIds.isEmpty()) { return new HashSet<String>(this.propertyIds); }
	...
	private boolean checkCategory(String unsupportedPropertyId) {
		...
		while (category != null) {
			if (this.propertyIds.contains(category)) { return true; }
	...
	/**
	 * Set a property value on the given resource for the given id and value. Make sure that the id is in the given set of requested ids.
	 * 
