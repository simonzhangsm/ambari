...
/**
 * This class analyzes a service's metrics to determine if additional metrics should be fetched. It's okay to maintain state here since these are done per-request.
 */
public class StackDefinedPropertyProvider implements PropertyProvider {
	...
	public StackDefinedPropertyProvider(Resource.Type type, JMXHostProvider jmxHostProvider, GangliaHostProvider gangliaHostProvider, StreamProvider streamProvider, String clusterPropertyId, String hostPropertyId, String componentPropertyId, String jmxStatePropertyId,
			PropertyProvider defaultJmxPropertyProvider, PropertyProvider defaultGangliaPropertyProvider) {
		
		if (null == clusterPropertyId) throw new NullPointerException("Cluster name property id cannot be null");
		if (null == componentPropertyId) throw new NullPointerException("Component name property id cannot be null");
	...
	public Set<Resource> populateResources(Set<Resource> resources, Request request, Predicate predicate) throws SystemException {
		...
		try {
			...
			if (gangliaMap.size() > 0) {
				GangliaPropertyProvider gpp = type.equals(Resource.Type.Component) ? new GangliaComponentPropertyProvider(gangliaMap, streamProvider, sslConfig, gangliaHostProvider, clusterNamePropertyId, componentNamePropertyId) : new GangliaHostComponentPropertyProvider(
	...
	private Map<String, PropertyInfo> getPropertyInfo(MetricDefinition def) {
		...
		for (Entry<String, Metric> entry : def.getMetrics().entrySet()) {
			...
			defs.put(entry.getKey(), new PropertyInfo(metric.getName(), metric.isTemporal(), metric.isPointInTime()));
		}
