...
/**
 * Agent Resource represents Ambari agent controller. It provides API for Ambari agents to get the cluster configuration changes as well as report the node attributes and state of services running the on the cluster nodes
 */
@Path("/")
public class AgentResource {
	 ...
	 * 
	 * @response.representation.200.doc This API is invoked by Ambari agent running on a cluster to register with the server.
	...
	public RegistrationResponse register(Register message, @Context HttpServletRequest req) throws WebApplicationException, InvalidStateTransitionException {
	 ...
	 * 
	 * @response.representation.200.doc This API is invoked by Ambari agent running on a cluster to update the state of various services running on the node.
	 * @response.representation.200.mediaType application/json
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces({ MediaType.APPLICATION_JSON })
	public HeartBeatResponse heartbeat(HeartBeat message) throws WebApplicationException {
	 ...
	 * 
	 * @response.representation.200.doc This API is invoked by Ambari agent running on a cluster to update the components category map of stack used by this cluster
	 * @response.representation.200.mediaType application/json
	 * @response.representation.408.doc Request Timed out
	...
	@GET
	@Produces({ MediaType.APPLICATION_JSON })
	public ComponentsResponse components(@PathParam("clusterName") String clusterName) {
		if (LOG.isDebugEnabled()) {
