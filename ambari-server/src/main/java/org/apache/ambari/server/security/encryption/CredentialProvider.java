...
public class CredentialProvider {
	...
	protected char[] chars = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k', 'm', 'n', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', '2', '3', '4',
			'5', '6', '7', '8', '9' };
	...
	public CredentialProvider(String masterKey, String masterKeyLocation, boolean isMasterKeyPersisted) throws AmbariException {
		...
		if (!masterKeyService.isMasterKeyInitialized()) { throw new AmbariException("Master key initialization failed."); }
	...
	public char[] getPasswordForAlias(String alias) throws AmbariException {
		if (isAliasString(alias)) return keystoreService.getCredential(getAliasFromString(alias));
	...
	public void addAliasToCredentialStore(String alias, String passwordString) throws AmbariException {
		...
		if (passwordString == null || passwordString.isEmpty()) throw new IllegalArgumentException("Empty or null password not allowed" + ".");
	...
	public static boolean isAliasString(String aliasStr) {
		if (aliasStr == null || aliasStr.isEmpty()) return false;
	...
	public static void main(String args[]) {
		if (args != null && args.length > 0) {
			...
			try {
				credentialProvider = new CredentialProvider(masterKey, configuration.getMasterKeyLocation(), configuration.isMasterKeyPersisted());
			} catch (Exception ex) {
				ex.printStackTrace();
				System.exit(1);
			}
