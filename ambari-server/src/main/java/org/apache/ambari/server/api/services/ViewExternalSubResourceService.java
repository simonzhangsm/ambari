...
public class ViewExternalSubResourceService extends BaseService {
	...
	public Response getResources(@Context HttpHeaders headers, @Context UriInfo ui) {
		return handleRequest(headers, null, ui, Request.Type.GET, createResource(viewName, instanceName));
	}
	...
	public Object getResource(@PathParam("resourceName") String resourceName) throws IOException {
		...
		if (service == null) { throw new IllegalArgumentException("A resource type " + resourceName + " for view instance " + viewName + "/" + instanceName + " can not be found."); }
