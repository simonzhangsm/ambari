...
public class ViewInstanceService extends BaseService {
	...
	/**
	 * Handles URL: /views/{viewID}/instances/{instanceID} Get a specific instance.
	 * 
	...
	@Path("{instanceName}")
	@Produces("text/plain")
	public Response getService(@Context HttpHeaders headers, @Context UriInfo ui, @PathParam("instanceName") String instanceName) {
		
		return handleRequest(headers, null, ui, Request.Type.GET, createResource(viewName, version, instanceName));
	}
	
	/**
	 * Handles URL: /views/{viewID}/instances Get all instances for a view.
	 * 
	 * @param headers http headers
	 * @param ui uri info
	...
	/**
	 * Handles: POST /views/{viewID}/instances/{instanceId} Create a specific instance.
	 * 
	...
	@Path("{instanceName}")
	@Produces("text/plain")
	public Response createService(String body, @Context HttpHeaders headers, @Context UriInfo ui, @PathParam("instanceName") String instanceName) {
		return handleRequest(headers, body, ui, Request.Type.POST, createResource(viewName, version, instanceName));
	}
	
	/**
	 * Handles: POST /views/{viewID}/instances Create multiple instances.
	 * 
	 * @param body http body
	 * @param headers http headers
	 * @param ui uri info
	...
	/**
	 * Handles: PUT /views/{viewID}/instances/{instanceId} Update a specific instance.
	 * 
	 * @param body http body
	 * @param headers http headers
	 * @param ui uri info
	...
	@Path("{instanceName}")
	@Produces("text/plain")
	public Response updateService(String body, @Context HttpHeaders headers, @Context UriInfo ui, @PathParam("instanceName") String instanceName) {
		
		return handleRequest(headers, body, ui, Request.Type.PUT, createResource(viewName, version, instanceName));
	}
	
	/**
	 * Handles: PUT /views/{viewID}/instances Update multiple instances.
	 * 
	 * @param body http body
	 * @param headers http headers
	 * @param ui uri info
	...
	/**
	 * Handles: DELETE /views/{viewID}/instances/{instanceId} Delete a specific instance.
	 * 
	 * @param headers http headers
	 * @param ui uri info
	 * @param instanceName instance id
	...
	@Path("{instanceName}")
	@Produces("text/plain")
	public Response deleteService(@Context HttpHeaders headers, @Context UriInfo ui, @PathParam("instanceName") String instanceName) {
	...
	public Object getResourceHandler(@PathParam("instanceName") String instanceName, @PathParam("resources") String resources) {
		
		ViewInstanceEntity instanceDefinition = ViewRegistry.getInstance().getInstanceDefinition(viewName, version, instanceName);
		
		if (instanceDefinition == null) { throw new IllegalArgumentException("A view instance " + viewName + "/" + instanceName + " can not be found."); }
		
		Object service = instanceDefinition.getService(resources);
		
		if (service == null) { throw new IllegalArgumentException("A resource type " + resources + " for view instance " + viewName + "/" + instanceName + " can not be found."); }
		return service;
	}
