...
public abstract class AbstractJDBCResourceProvider<E extends Enum<E>> extends AbstractResourceProvider {
	 ...
	 */
	protected AbstractJDBCResourceProvider(Set<String> propertyIds, Map<Type, String> keyPropertyIds) {
	 ...
	 * 
	 * @param resource resource object to set the property on
	 * @param propertyId the property id to retrieve from the result set
	 * @param rs the result set
	 * @param requestedIds the requested ids
	 * @throws SQLException if property id cannot be retrieved from the result set
	 */
	protected void setString(Resource resource, String propertyId, ResultSet rs, Set<String> requestedIds) throws SQLException {
		if (requestedIds.contains(propertyId)) setResourceProperty(resource, propertyId, rs.getString(dbFields.get(propertyId).toString()), requestedIds);
	}
	...
	protected void setInt(Resource resource, String propertyId, ResultSet rs, Set<String> requestedIds) throws SQLException {
		if (requestedIds.contains(propertyId)) setResourceProperty(resource, propertyId, rs.getInt(dbFields.get(propertyId).toString()), requestedIds);
	}
	...
	protected void setLong(Resource resource, String propertyId, ResultSet rs, Set<String> requestedIds) throws SQLException {
		if (requestedIds.contains(propertyId)) setResourceProperty(resource, propertyId, rs.getLong(dbFields.get(propertyId).toString()), requestedIds);
	}
	...
	protected String getDBFieldString(Set<String> requestedIds) {
		...
		for (String s : requestedIds)
			if (dbFields.containsKey(s)) tmp[i++] = dbFields.get(s).toString();
