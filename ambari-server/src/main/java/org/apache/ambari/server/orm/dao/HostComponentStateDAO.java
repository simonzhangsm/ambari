...
public class HostComponentStateDAO {
	...
	public List<HostComponentStateEntity> findAll() {
		TypedQuery<HostComponentStateEntity> query = entityManagerProvider.get().createQuery("SELECT hsc from HostComponentStateEntity hsc", HostComponentStateEntity.class);
		try {
