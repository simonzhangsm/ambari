...
public interface Resource {
	...
	/**
	 * Obtain the properties contained by this group in a map structure. The category/property hierarchy is flattened into a map where each key is the absolute category name and the corresponding value is a map of properties(name/value pairs) for that category.
	 * 
	...
	public enum InternalType {
		Cluster, Service, Host, Component, HostComponent, Configuration, ConfigGroup, Action, Request, RequestSchedule, Task, User, Stack, StackVersion, OperatingSystem, Repository, StackService, StackConfiguration, StackServiceComponent, StackServiceComponentDependency, DRFeed, DRTargetCluster, DRInstance, Workflow, Job, TaskAttempt, RootService, RootServiceComponent, RootServiceHostComponent, View, ViewVersion, ViewInstance, Blueprint, HostComponentProcess;
	...
	public final class Type implements Comparable<Type> {
		...
		public InternalType getInternalType() {
			if (isInternalType()) { return InternalType.values()[ordinal]; }
		...
		public static Type valueOf(String name) {
			...
			if (type == null) { throw new IllegalArgumentException(name + " is not a type."); }
			return type;
		}
