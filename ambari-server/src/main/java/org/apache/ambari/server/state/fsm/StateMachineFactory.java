...
final public class StateMachineFactory<OPERAND, STATE extends Enum<STATE>, EVENTTYPE extends Enum<EVENTTYPE>, EVENT> {
	...
	/**
	 * @return a NEW StateMachineFactory just like {@code this} with the current transition added as a new legal transition. This overload has no hook object. Note that the returned StateMachineFactory is a distinct object. This method is part of the API.
	...
	/**
	 * @return a NEW StateMachineFactory just like {@code this} with the current transition added as a new legal transition Note that the returned StateMachineFactory is a distinct object. This method is part of the API.
	 * @param preState pre-transition state
	 * @param postState post-transition state
	 * @param eventTypes List of stimuli for the transitions
	...
	public StateMachineFactory<OPERAND, STATE, EVENTTYPE, EVENT> addTransition(STATE preState, STATE postState, EVENTTYPE eventType, SingleArcTransition<OPERAND, EVENT> hook) {
		return new StateMachineFactory(this, new ApplicableSingleOrMultipleTransition(preState, eventType, new SingleInternalArc(postState, hook)));
	}
	...
	public StateMachineFactory<OPERAND, STATE, EVENTTYPE, EVENT> addTransition(STATE preState, Set<STATE> postStates, EVENTTYPE eventType, MultipleArcTransition<OPERAND, EVENT, STATE> hook) {
		return new StateMachineFactory(this, new ApplicableSingleOrMultipleTransition(preState, eventType, new MultipleInternalArc(postStates, hook)));
	}
	
	/**
	 * @return a StateMachineFactory just like {@code this}, except that if you won't need any synchronization to build a state machine Note that the returned StateMachineFactory is a distinct object. This method is part of the API. The only way you could distinguish the returned
	 *         StateMachineFactory from {@code this} would be by measuring the performance of the derived {@code StateMachine} you can get from it. Calling this is optional. It doesn't change the semantics of the factory, if you call it then when you use the factory there is no
	 *         synchronization.
	 */
	...
	private interface Transition<OPERAND, STATE extends Enum<STATE>, EVENTTYPE extends Enum<EVENTTYPE>, EVENT> {
		STATE doTransition(OPERAND operand, STATE oldState, EVENT event, EVENTTYPE eventType) throws InvalidStateTransitionException;
	}
	...
	private class MultipleInternalArc implements Transition<OPERAND, STATE, EVENTTYPE, EVENT> {
		...
		public STATE doTransition(OPERAND operand, STATE oldState, EVENT event, EVENTTYPE eventType) throws InvalidStateTransitionException {
			...
			if (!validPostStates.contains(postState)) { throw new InvalidStateTransitionException(oldState, eventType); }
			return postState;
		}
	}
	
	/*
	 * @return a {@link StateMachine} that starts in {@code initialState} and whose {@link Transition} s are applied to {@code operand} . This is part of the API.
	 * @param operand the object upon which the returned {@link StateMachine} will operate.
	 * @param initialState the state in which the returned {@link StateMachine} will start.
	 */
	 ...
	 * @return Graph object generated public Graph generateStateGraph(String name) { maybeMakeStateMachineTable(); Graph g = new Graph(name); for (STATE startState : stateMachineTable.keySet()) { Map<EVENTTYPE, Transition<OPERAND, STATE, EVENTTYPE, EVENT>> transitions =
	 * stateMachineTable.get(startState); for (Entry<EVENTTYPE, Transition<OPERAND, STATE, EVENTTYPE, EVENT>> entry : transitions.entrySet()) { Transition<OPERAND, STATE, EVENTTYPE, EVENT> transition = entry.getValue(); if (transition instanceof
	 ...
	 * entry.getKey().toString()); } else if (transition instanceof StateMachineFactory.MultipleInternalArc) { StateMachineFactory.MultipleInternalArc ma = (StateMachineFactory.MultipleInternalArc) transition; Iterator<STATE> iter = ma.validPostStates.iterator(); while
	 * (iter.hasNext()) { Graph.Node fromNode = g.getNode(startState.toString()); Graph.Node toNode = g.getNode(iter.next().toString()); fromNode.addEdge(toNode, entry.getKey().toString()); } } } } return g; }
	 */
}
