...
public class BatchRequestJob extends AbstractLinearExecutionJob {
	...
	@Inject
	public BatchRequestJob(ExecutionScheduleManager executionScheduleManager, @Named("statusCheckInterval") long statusCheckInterval) {
	...
	protected void doWork(Map<String, Object> properties) throws AmbariException {
		...
		if (executionId == null || batchId == null) { throw new AmbariException("Unable to retrieve persisted batch request" + ", execution_id = " + executionId + ", batch_id = " + batchId); }
		...
		if (requestId != null) {
			...
			throw new AmbariException("Task failure tolerance limit exceeded" + ", execution_id = " + executionId + ", processed batch_id = " + batchId + ", failed tasks = " + aggregateCounts.get(BATCH_REQUEST_FAILED_TASKS_KEY) + ", total tasks completed = "
					+ aggregateCounts.get(BATCH_REQUEST_TOTAL_TASKS_KEY)); }
		}
	}
	
	@Override
	protected void finalizeExecution(Map<String, Object> properties) throws AmbariException {
