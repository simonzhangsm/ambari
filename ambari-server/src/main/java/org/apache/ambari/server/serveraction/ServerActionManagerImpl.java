...
public class ServerActionManagerImpl implements ServerActionManager {
	...
	private final static Logger LOG = LoggerFactory.getLogger(ServerActionManagerImpl.class);
	...
	public void executeAction(String actionName, Map<String, String> payload) throws AmbariException {
		LOG.info("Executing server action : " + actionName + " with payload " + payload);
	...
	private void updateClusterStackVersion(Map<String, String> payload) throws AmbariException {
		if (payload == null || !payload.containsKey(ServerAction.PayloadName.CLUSTER_NAME) || !payload.containsKey(ServerAction.PayloadName.CURRENT_STACK_VERSION)) { throw new AmbariException("Invalid payload."); }
