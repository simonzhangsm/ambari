...
public class RequestService extends BaseService {
	...
	/**
	 * Handles URL: /clusters/{clusterID}/requests/{requestID} Get a specific request.
	 * 
	...
	@Path("{requestId}")
	@Produces("text/plain")
	public Response getRequest(@Context HttpHeaders headers, @Context UriInfo ui, @PathParam("requestId") String requestId) {
		
		return handleRequest(headers, null, ui, Request.Type.GET, createRequestResource(m_clusterName, requestId));
	}
	
	/**
	 * Handles URL: /clusters/{clusterId}/requests Get all requests for a cluster.
	 * 
	 * @param headers http headers
	 * @param ui uri info
	...
	public Response getRequests(@Context HttpHeaders headers, @Context UriInfo ui) {
		return handleRequest(headers, null, ui, Request.Type.GET, createRequestResource(m_clusterName, null));
	}
	...
	/**
	 * Handles: POST /clusters/{clusterId}/requests Create multiple services.
	 * 
