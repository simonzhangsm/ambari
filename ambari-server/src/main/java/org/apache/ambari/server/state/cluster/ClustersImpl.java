...
public class ClustersImpl implements Clusters {
	...
	public void addCluster(String clusterName) throws AmbariException {
		...
		if (clusters.containsKey(clusterName)) { throw new DuplicateResourceException("Attempted to create a Cluster which already exists" + ", clusterName=" + clusterName); }
	...
	public Cluster getCluster(String clusterName) throws AmbariException {
		...
		r.lock();
		try {
			if (!clusters.containsKey(clusterName)) { throw new ClusterNotFoundException(clusterName); }
	...
	public Cluster getClusterById(long id) throws AmbariException {
		...
		r.lock();
		try {
			if (!clustersById.containsKey(id)) { throw new ClusterNotFoundException("clusterID=" + id); }
	...
	public Set<Cluster> getClustersForHost(String hostname) throws AmbariException {
		...
		try {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Looking up clusters for hostname" + ", hostname=" + hostname + ", mappedClusters=" + hostClusterMap.get(hostname).size());
			}
	...
	public Host getHost(String hostname) throws AmbariException {
		...
		r.lock();
		try {
			if (!hosts.containsKey(hostname)) { throw new HostNotFoundException(hostname); }
	...
	public void addHost(String hostname) throws AmbariException {
		checkLoaded();
		String duplicateMessage = "Duplicate entry for Host" + ", hostName= " + hostname;
		
		if (hosts.containsKey(hostname)) { throw new AmbariException(duplicateMessage); }
		r.lock();
	...
	private boolean isOsSupportedByClusterStack(Cluster c, Host h) throws AmbariException {
		...
		if (repos == null || repos.isEmpty()) { return false; }
					...
					for (String clusterName : hostClusterNames) {
						if (clusterName != null && !clusterName.isEmpty()) {
							...
							for (Cluster c : hostClusterMap.get(hostname)) {
								if (c.getClusterName().equals(clusterName)) { throw new DuplicateResourceException("Attempted to create a host which already exists: clusterName=" + clusterName + ", hostName=" + hostname); }
							}
							
							if (!isOsSupportedByClusterStack(cluster, host)) {
								String message = "Trying to map host to cluster where stack does not" + " support host's os type" + ", clusterName=" + clusterName + ", clusterStackId=" + cluster.getDesiredStackVersion().getStackId() + ", hostname=" + hostname + ", hostOsFamily="
										+ host.getOsFamily();
								LOG.warn(message);
	...
	private Map<String, Host> getHostsMap(Collection<String> hostSet) throws HostNotFoundException {
		...
		try {
			for (String host : hostSet) {
				if (!hosts.containsKey(host)) throw new HostNotFoundException(host);
				else
	...
	@Transactional
	private Map<String, Cluster> getClustersMap(Collection<String> clusterSet) throws ClusterNotFoundException {
		checkLoaded();
		...
		try {
			for (String c : clusterSet) {
				if (c != null) {
					if (!clusters.containsKey(c)) throw new ClusterNotFoundException(c);
					else
	...
	public void deleteCluster(String clusterName) throws AmbariException {
		...
		try {
			Cluster cluster = getCluster(clusterName);
			if (!cluster.canBeRemoved()) { throw new AmbariException("Could not delete cluster" + ", clusterName=" + clusterName); }
	...
	public void deleteHost(String hostname) throws AmbariException {
		...
		if (!hosts.containsKey(hostname)) return;
