...
public class ActionDefinitionManager {
	...
	private final static Logger LOG = LoggerFactory.getLogger(ActionDefinitionManager.class);
			...
			for (File definitionFile : customActionDefinitionFiles) {
				...
				for (ActionDefinitionSpec ad : adx.actionDefinitions()) {
					...
					if (isValidActionDefinition(ad, actionType, defaultTimeout, errorReason)) {
						...
						actionDefinitionMap.put(ad.getActionName(), new ActionDefinition(ad.getActionName(), actionType, ad.getInputs(), ad.getTargetService(), ad.getTargetComponent(), ad.getDescription(), targetType, defaultTimeout));
	...
	private boolean isValidActionDefinition(ActionDefinitionSpec ad, ActionType actionType, Short defaultTimeout, StringBuilder reason) {
