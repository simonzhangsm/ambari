...
/**
 * Handles SPNego authentication as a client of hadoop service, caches hadoop.auth cookie returned by hadoop service on successful SPNego authentication. Refreshes hadoop.auth cookie on demand if the cookie has expired.
 */
public class AppCookieManager {
	 ...
	 * 
	 * @param endpoint the URL of the Hadoop service
	 * @param refresh flag indicating wehther to refresh the cookie, if <code>true</code>, we do a new SPNego authentication and refresh the cookie even if the cookie already exists in local cache
	 * @return hadoop.auth cookie value
	 * @throws IOException in case of problem getting hadoop.auth cookie
	 */
	public String getAppCookie(String endpoint, boolean refresh) throws IOException {
		...
		if (!refresh) {
			...
			if (appCookie != null) { return appCookie; }
		}
		...
		client.getCredentialsProvider().setCredentials(new AuthScope(/* host */null, /* port */-1, /* realm */null), EMPTY_JAAS_CREDENTIALS);
			...
			for (HeaderElement element : elements) {
				...
				if (cookieName.equals(HADOOP_AUTH)) {
					if (element.getValue() != null) {
						...
						if (!trimmedVal.isEmpty()) { return trimmedVal; }
					}
				}
			}
		}
