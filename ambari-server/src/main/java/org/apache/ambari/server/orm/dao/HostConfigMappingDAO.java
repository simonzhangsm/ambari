...
public class HostConfigMappingDAO {
	...
	public HostConfigMapping findSelectedByType(final long clusterId, String hostName, final String type) {
		...
		if (!hostConfigMappingByHost.containsKey(hostName)) return null;
	...
	public Set<HostConfigMapping> findSelectedByHosts(long clusterId, Collection<String> hostNames) {
		...
		if (hostNames == null || hostNames.isEmpty()) { return Collections.emptySet(); }
	...
	@RequiresSession
	public Map<String, List<HostConfigMapping>> findSelectedHostsByTypes(final long clusterId, Collection<String> types) {
	...
	public void removeHost(final long clusterId, String hostName) {
		...
		// delete from db
		TypedQuery<HostConfigMappingEntity> query = entityManagerProvider.get().createQuery("SELECT entity FROM HostConfigMappingEntity entity " + "WHERE entity.clusterId = ?1 AND entity.hostName = ?2", HostConfigMappingEntity.class);
