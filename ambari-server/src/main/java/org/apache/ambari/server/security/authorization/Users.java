...
public class Users {
	...
	public User getLocalUser(String userName) throws AmbariException {
		...
		if (userEntity == null) { throw new AmbariException("User doesn't exist"); }
		return new User(userEntity);
	}
	...
	public synchronized void modifyPassword(String userName, String currentUserPassword, String newPassword) throws AmbariException {
		...
		if (currentUserName == null) { throw new AmbariException("Authentication required. Please sign in."); }
	...
	public synchronized void removeUser(User user) throws AmbariException {
		...
		if (userEntity != null) {
			if (!isUserCanBeRemoved(userEntity)) { throw new AmbariException("Could not remove user " + userEntity.getUserName() + ". System should have at least one user with administrator role."); }
	...
	public synchronized void addRoleToUser(User user, String role) throws AmbariException {
		...
		UserEntity userEntity = userDAO.findByPK(user.getUserId());
		if (userEntity == null) { throw new AmbariException("User " + user + " doesn't exist"); }
	...
	public synchronized void removeRoleFromUser(User user, String role) throws AmbariException {
		...
		RoleEntity roleEntity = roleDAO.findByName(role);
		if (roleEntity == null) { throw new AmbariException("Role " + role + " doesn't exist"); }
		if (role.equals(getAdminRole())) {
			if (!isUserCanBeRemoved(userEntity)) { throw new AmbariException("Could not remove admin role from user " + userEntity.getUserName() + ". System should have at least one user with administrator role."); }
		}
