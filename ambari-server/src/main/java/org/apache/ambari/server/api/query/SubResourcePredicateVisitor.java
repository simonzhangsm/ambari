...
/**
 * A predicate visitor used to extract the sub resource elements from a given predicate and produce a new predicate that can be evaluated with a sub resource of the given category. For example, given the category 'components' the predicate ... ServiceInfo/service_name = HBASE AND
 * components/ServiceComponentInfo/category = SLAVE AND components/host_components/metrics/cpu/cpu_num >= 1 ... will produce the new predicate ... ServiceComponentInfo/category = SLAVE AND host_components/metrics/cpu/cpu_num >= 1
 */
