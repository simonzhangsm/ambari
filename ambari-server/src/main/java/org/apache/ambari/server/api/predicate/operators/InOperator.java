...
/**
 * IN relational operator. This is a binary operator which takes a comma delimited right operand and creates equals predicates with the left operand and each right operand token. The equals predicates are combined with an OR predicate.
 */
public class InOperator extends AbstractOperator implements RelationalOperator {
	...
	public Predicate toPredicate(String prop, String val) throws InvalidQueryException {
		...
		if (val == null) { throw new InvalidQueryException("IN operator is missing a required right operand."); }
