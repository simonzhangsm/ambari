...
class BSRunner extends Thread {
	...
	public BSRunner(BootStrapImpl impl, SshHostInfo sshHostInfo, String bootDir, String bsScript, String agentSetupScript, String agentSetupPassword, int requestId, long timeout, String hostName, boolean isVerbose, String clusterOsType, String projectVersion, int serverPort) {
	...
	private void createRunDir() throws IOException {
		if (!bootDir.exists()) {
			...
			if (!bootDir.mkdirs()) { throw new IOException("Cannot create " + bootDir); }
		}
		...
		if (!requestIdDir.mkdirs()) { throw new IOException("Cannot create " + requestIdDir); }
	}
	...
	public void run() {
		...
		try {
			...
			/*
			 * Running command: script hostlist bsdir user sshkeyfile
			 */
			shellCommand[0] = "sh";
			shellCommand[1] = "-c";
			...
			LOG.info("Host= " + hostString + " bs=" + this.bsScript + " requestDir=" + requestIdDir + " user=" + user + " keyfile=" + this.sshKeyFile + " passwordFile " + this.passwordFile + " server=" + this.ambariHostname + " version=" + projectVersion + " serverPort="
					+ this.serverPort);
