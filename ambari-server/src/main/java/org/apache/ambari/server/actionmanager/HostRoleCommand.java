...
/**
 * This class encapsulates the information for an task on a host for a particular role which action manager needs. It doesn't capture actual command and parameters, but just the stuff enough for action manager to track the request.
 */
public class HostRoleCommand {
	...
	public HostRoleCommand(String host, Role role, ServiceComponentHostEvent event, RoleCommand command) {
		this.hostName = host;
		this.role = role;
	...
	public void setTaskId(long taskId) {
		if (this.taskId != -1) { throw new RuntimeException("Attempt to set taskId again, not allowed"); }
		this.taskId = taskId;
	...
	public ExecutionCommandWrapper getExecutionCommandWrapper() {
		if (taskId != -1 && executionCommandWrapper == null) {
			...
			if (commandEntity == null) { throw new RuntimeException("Invalid DB state, broken one-to-one relation for taskId=" + taskId); }
			executionCommandWrapper = new ExecutionCommandWrapper(new String(commandEntity.getCommand()));
		}
	...
	@Override
	public int hashCode() {
		return (hostName.toString() + role.toString() + roleCommand.toString()).hashCode();
	}
	
	@Override
	public boolean equals(Object other) {
		if (!(other instanceof HostRoleCommand)) { return false; }
		HostRoleCommand o = (HostRoleCommand) other;
		return (this.role.equals(o.role) && this.hostName.equals(o.hostName) && this.roleCommand.equals(o.roleCommand));
	}
