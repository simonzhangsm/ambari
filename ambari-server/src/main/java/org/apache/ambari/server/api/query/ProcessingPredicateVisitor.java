...
/**
 * A predicate visitor used to process a predicate to do the following : 1) Create a new predicate with all sub-resource elements contained in the visited predicate removed (converted to AlwaysPredicate). 2) Create a set of any sub-resource subResourceCategories that were
 * referenced in the predicate being visited (i.e. components, host_components, etc). 3) Create a set of any sub-resource subResourceProperties that were referenced in the predicate being visited. For example, the service level query the predicate ... ServiceInfo/service_name =
 * HBASE AND components/ServiceComponentInfo/category = SLAVE AND components/host_components/metrics/cpu/cpu_num >= 1 ... will produce ... Predicate : ServiceInfo/service_name = HBASE Sub-resource subResourceCategories : {components} Sub-resource subResourceProperties :
 * {components/ServiceComponentInfo/category, components/host_components/metrics/cpu/cpu_num}
 */
