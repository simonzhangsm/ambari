...
public class RequestExecutionImpl implements RequestExecution {
	...
	public RequestScheduleResponse convertToResponse() {
		...
		try {
			RequestScheduleResponse response = new RequestScheduleResponse(getId(), getClusterName(), getDescription(), getStatus(), getLastExecutionStatus(), getBatch(), getSchedule(), requestScheduleEntity.getCreateUser(), DateUtils.convertToReadableTime(requestScheduleEntity
					.getCreateTimestamp()), requestScheduleEntity.getUpdateUser(), DateUtils.convertToReadableTime(requestScheduleEntity.getUpdateTimestamp()));
			return response;
		} finally {
	...
	public String getCreateTime() {
		return DateUtils.convertToReadableTime(requestScheduleEntity.getCreateTimestamp());
	}
	
	@Override
	public String getUpdateTime() {
		return DateUtils.convertToReadableTime(requestScheduleEntity.getUpdateTimestamp());
	}
	...
	public BatchRequest getBatchRequest(long batchId) {
		for (BatchRequest batchRequest : batch.getBatchRequests()) {
			if (batchId == batchRequest.getOrderId()) { return batchRequest; }
		}
		return null;
	}
