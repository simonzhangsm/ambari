...
public class JMXPropertyProvider extends AbstractPropertyProvider {
	...
	static {
		...
		ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(THREAD_POOL_CORE_SIZE, THREAD_POOL_MAX_SIZE, THREAD_POOL_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS, queue);
	...
	protected final static Logger LOG = LoggerFactory.getLogger(JMXPropertyProvider.class);
	...
	/**
	 * The amount of time that this provider will wait for JMX metric values to be returned from the JMX sources. If no results are returned for this amount of time then the request to populate the resources will fail.
	 */
	 ...
	 */
	public JMXPropertyProvider(Map<String, Map<String, PropertyInfo>> componentMetrics, StreamProvider streamProvider, JMXHostProvider jmxHostProvider, String clusterNamePropertyId, String hostNamePropertyId, String componentNamePropertyId, String statePropertyId,
	 ...
	 * @return the spec
	 */
	protected String getSpec(String protocol, String hostName, String port, String componentName) {
		if (null == componentName || !componentName.equals(STORM_REST_API)) return protocol + "://" + hostName + ":" + port + "/jmx";
		else
	 ...
	 */
	private Callable<Resource> getPopulateResourceCallable(final Resource resource, final Request request, final Predicate predicate) {
	 ...
	 */
	private Resource populateResource(Resource resource, Request request, Predicate predicate) throws SystemException {
		...
		if (statePropertyId != null) {
			...
			if (state != null && !healthyStates.contains(state)) { return resource; }
		}
	 ...
	 */
	private void getHadoopMetricValue(InputStream in, Set<String> ids, Resource resource, Request request) throws IOException {
	...
	/**
	 * TODO: Refactor Storm-specific metrics fetching
	 */
	private void getStormMetricValue(InputStream in, Set<String> ids, Resource resource, Request request) throws IOException {
	...
	private String getHost(Resource resource, String clusterName, String componentName) throws SystemException {
		return hostNamePropertyId == null ? jmxHostProvider.getHostName(clusterName, componentName) : (String) resource.getPropertyValue(hostNamePropertyId);
	}
	...
	private static void rethrowSystemException(Throwable throwable) throws SystemException {
		...
		if (throwable instanceof SystemException) { throw (SystemException) throwable; }
