...
/**
 * This class encapsulates the action scheduler thread. Action schedule frequently looks at action database and determines if there is an action that can be scheduled.
 */
class ActionScheduler implements Runnable {
	...
	/**
	 * true if scheduler should run ASAP. We need this flag to avoid sleep in situations, when we receive awake() request during running a scheduler iteration.
	 */
	...
	public ActionScheduler(long sleepTimeMilliSec, long actionTimeoutMilliSec, ActionDBAccessor db, ActionQueue actionQueue, Clusters fsmObject, int maxAttempts, HostsMap hostsMap, ServerActionManager serverActionManager, UnitOfWork unitOfWork, Configuration configuration) {
		...
		this.clusterHostInfoCache = CacheBuilder.newBuilder().expireAfterAccess(5, TimeUnit.MINUTES).build();
	...
	/**
	 * Should be called from another thread when we want scheduler to make a run ASAP (for example, to process desired configs of SCHs). The method is guaranteed to return quickly.
	 */
	public void awake() {
			...
			for (Stage s : stages) {
				...
				for (ExecutionCommand cmd : commandsToSchedule) {
					if (Role.valueOf(cmd.getRole()).equals(Role.AMBARI_SERVER_ACTION)) {
						/**
						 * We don't forbid executing any stages in parallel with AMBARI_SERVER_ACTION. That should be OK as AMBARI_SERVER_ACTION is not used as of now. The general motivation has been to update Request status when last task associated with the Request is finished.
						 */
	...
	private void reportServerActionSuccess(Stage stage, ExecutionCommand cmd) {
		...
		report.setStdErr("");
		db.updateHostRoleState(cmd.getHostname(), stage.getRequestId(), stage.getStageId(), cmd.getRole(), report);
	}
	...
	/**
	 * @return Stats for the roles in the stage. It is used to determine whether stage has succeeded or failed. Side effects: This method processes command timeouts and retry attempts, and adds new (pending) execution commands to commandsToSchedule list.
	 */
	private Map<String, RoleStats> processInProgressStage(Stage s, List<ExecutionCommand> commandsToSchedule) throws AmbariException {
		...
		for (String host : s.getHosts()) {
			...
			for (ExecutionCommandWrapper wrapper : commandWrappers) {
				...
				if (scHosts != null && !scHosts.containsKey(host)) {
					String message = String.format("Service component host not found when trying to " + "schedule an execution command. " + "The most probable reason " + "for that is that host component " + "has been deleted recently. "
							+ "The command has been aborted and dequeued. " + "Execution command details: " + "cluster=%s; host=%s; service=%s; component=%s; " + "cmdId: %s; taskId: %s; roleCommand: %s", c.getClusterName(), host,
							svcComp == null ? "null" : svcComp.getServiceName(), svcComp == null ? "null" : svcComp.getName(), c.getCommandId(), c.getTaskId(), c.getRoleCommand());
					LOG.warn(message);
	...
	private void transitionToFailedState(String clusterName, String serviceName, String componentName, String hostname, long timestamp, boolean ignoreTransitionException) {
		...
		try {
			...
			ServiceComponentHostOpFailedEvent timeoutEvent = new ServiceComponentHostOpFailedEvent(componentName, hostname, timestamp);
	...
	private boolean timeOutActionNeeded(HostRoleStatus status, Stage stage, Host host, String role, long currentTime, long taskTimeout) throws AmbariException {
		if ((!status.equals(HostRoleStatus.QUEUED)) && (!status.equals(HostRoleStatus.IN_PROGRESS))) { return false; }
		...
		if (currentTime > stage.getLastAttemptTime(host.getHostName(), role) + taskTimeout) { return true; }
		return false;
	}
	
	private void scheduleHostRole(Stage s, ExecutionCommand cmd) throws InvalidStateTransitionException, AmbariException {
		long now = System.currentTimeMillis();
	...
	static class RoleStats {
		...
		/**
		 * Role failure means role is no longer in progress and success factor is not met.
		 */
