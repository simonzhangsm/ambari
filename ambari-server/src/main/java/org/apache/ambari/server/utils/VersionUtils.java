...
/**
 * Provides various utility functions to be used for version handling. The compatibility matrix between server, agent, store can also be maintained in this class. Currently, exact match is required between all the three.
 */
public class VersionUtils {
	 ...
	 * @param version1
	 * @param version2
	 * @param maxLengthToCompare The maximum length to compare - 2 means only Major and Minor 0 to compare the whole version strings
	...
	public static int compareVersions(String version1, String version2, int maxLengthToCompare) throws IllegalArgumentException {
		...
		if (version1 == null || version1.isEmpty()) { throw new IllegalArgumentException("version1 cannot be null or empty"); }
		if (version2 == null || version2.isEmpty()) { throw new IllegalArgumentException("version2 cannot be null or empty"); }
		if (maxLengthToCompare < 0) { throw new IllegalArgumentException("maxLengthToCompare cannot be less than 0"); }
		...
		for (int i = 0; i < length; i++) {
			String stack1Part = i < version1Parts.length ? version1Parts[i] : "0";
			String stack2Part = i < version2Parts.length ? version2Parts[i] : "0";
			
			while (stack1Part.length() > stack2Part.length())
				stack2Part = "0".concat(stack2Part);
			
			while (stack2Part.length() > stack1Part.length())
				stack1Part = "0".concat(stack1Part);
			
			int rst = stack1Part.compareTo(stack2Part);
			if (rst < 0) { return -1; }
			if (rst > 0) { return 1; }
		}
	...
	public static String getVersionSubstring(String version) {
		...
		if (versionParts.length < 3) { throw new IllegalArgumentException("Invalid version number"); }
