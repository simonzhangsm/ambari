...
/**
 * Scans a query expression and generates an array of tokens. Each token contains type and value information. First, the query expression is broken down into string tokens using a regular expression which splits on a set of deliminators which includes operators and brackets.
...
public class QueryLexer {
	 ...
	 */
	private static final String[] ALL_DELIMS = { ".in\\(", ".isEmpty\\(", "<=", ">=", "!=", "=", "<", ">", "&", "|", "!", "(", ")" };
	...
	public Token[] tokens(String exp) throws InvalidQueryException {
		...
		for (String tok : parseStringTokens(exp)) {
			...
			if (!processed) { throw new InvalidQueryException("Invalid Query Token: token='" + tok + "\', previous token type=" + ctx.getLastTokenType()); }
		}
		return ctx.getTokenList().toArray(new Token[ctx.getTokenList().size()]);
	}
	
	/**
	 * Uses a regular expression to scan a query expression and produce a list of string tokens. These tokens are the exact strings that exist in the original syntax.
	 * 
	...
	private class ScanContext {
		...
		/**
		 * Get the current property operand value. This is used to hold the property operand name until it is added since, the following relational operator token is added first.
		 * 
		...
		/**
		 * Set the current property operand value. This is used to hold the property operand name until it is added since, the following relational operator token is added first.
		 */
	...
	/**
	 * Token handler base class. Token handlers are responsible for processing specific token type.
	 */
