...
public class ClusterBlueprintRenderer extends BaseRenderer implements Renderer {
	...
	private Resource createBlueprintResource(TreeNode<Resource> clusterNode) {
		...
		String clusterName = (String) clusterResource.getPropertyValue(PropertyHelper.getPropertyId("Clusters", "cluster_name"));
		...
		blueprintResource.setProperty("host_groups", processHostGroups(clusterNode.getChild("hosts")));
	...
	private static class HostGroup {
		...
		/**
		 * Factory method for obtaining a host group instance. Parses a host tree node for host related information.
		 * 
		...
		public static HostGroup parse(TreeNode<Resource> host) {
			...
			for (TreeNode<Resource> component : components.getChildren()) {
				group.getComponents().add((String) component.getObject().getPropertyValue("HostRoles/component_name"));
			}
			
			group.addAmbariComponentIfLocalhost((String) host.getObject().getPropertyValue(PropertyHelper.getPropertyId("Hosts", "host_name")));
