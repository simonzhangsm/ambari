...
public class QueryCreateHandler extends BaseManagementHandler {
	...
	public Result handleRequest(Request request) {
		...
		if (mapProperties.size() != 1) { return createInvalidRequestResult(mapProperties.size() == 0 ? "A minimum of one sub-resource must be specified for creation." : "Multiple sub-resource types may not be created in the same request."); }
	...
	/**
	 * Build the property set for all sub-resource to be created. This includes determining the sub-resource type and creating a property set for each matching parent.
	 * 
	 ...
	 * @throws IllegalArgumentException if no sub-resource type was specified or it is not a valid sub-resource of the parent.
	 */
	 ...
	 * @throws IllegalArgumentException if the specified sub-resource name is empty or it is not a valid sub-resource of the parent.
	 */
	private Resource.Type getCreateType(ResourceInstance resource, String subResourceName) throws IllegalArgumentException {
		if (subResourceName == null || subResourceName.equals("")) { throw new IllegalArgumentException("A sub-resource name must be supplied."); }
		ResourceInstance res = resource.getSubResources().get(subResourceName);
		
		if (res == null) { throw new IllegalArgumentException("The specified sub-resource name is not valid: '" + subResourceName + "'."); }
