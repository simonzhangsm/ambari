...
public class MasterKeyServiceImpl implements MasterKeyService {
	...
	public MasterKeyServiceImpl(String masterFileLocation, boolean isPersisted) {
		if (masterFileLocation == null || masterFileLocation.isEmpty()) throw new IllegalArgumentException("Master Key location not provided.");
		if (isPersisted) {
			File masterFile = new File(masterFileLocation);
			if (masterFile.exists()) {
				try {
					...
					LOG.error("Cannot intitialize master key from file: " + masterFileLocation + "\n" + ex);
				}
			} else {
				LOG.error("Cannot find master key at specified location " + masterFileLocation);
			}
		} else {
			...
			if (key != null) {
				...
				LOG.debug("Master key is not provided as a System property or an " + "environment varialble.");
			}
		}
	}
				...
				if (keyPath != null && !keyPath.isEmpty()) {
					...
					if (keyFile.exists()) {
						try {
							...
							if (this.master != null) key = new String(this.master);
	...
	private void chmod(String args, File file) throws IOException {
		if (isUnixEnv()) {
			...
			if (args == null || file == null) throw new IOException("nullArg");
			if (!file.exists()) throw new IOException("fileNotFound");
	...
	private String generateMasterKey() {
		char[] chars = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k', 'm', 'n', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', '2', '3', '4', '5', '6',
				'7', '8', '9' };
	...
	public static void main(String args[]) {
		...
		if (args != null && args.length > 0) {
			masterKey = args[0];
			if (args.length > 1) masterKeyLocation = args[1];
			if (args.length > 2 && !args[2].isEmpty()) persistMasterKey = args[2].toLowerCase().equals("true");
		}
		MasterKeyService masterKeyService = new MasterKeyServiceImpl(masterKey, masterKeyLocation, persistMasterKey);
