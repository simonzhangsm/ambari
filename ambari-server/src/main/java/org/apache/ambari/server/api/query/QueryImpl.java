...
public class QueryImpl implements Query, ResourceInstance {
	...
	/**
	 * Definition for the resource type. The definition contains all information specific to the resource type.
	 */
	...
	/**
	 * Sub-resources of the resource which is being operated on. Should only be added via {@link #addSubResource(String, QueryImpl)}
	 */
	private final Map<String, QueryImpl> requestedSubResources = new HashMap<String, QueryImpl>();
	
	/**
	 * Sub-resource instances of this resource. Map of resource name to resource instance.
	 */
	...
	/**
	 * Associated renderer. The default renderer is used unless an alternate renderer is specified for the request. The renderer is responsible for determining which properties are selected as well as the overall structure of the result.
	 */
	...
	public boolean equals(Object o) {
		...
		return clusterController.equals(query.clusterController) && !(pageRequest != null ? !pageRequest.equals(query.pageRequest) : query.pageRequest != null) && requestedProperties.equals(query.requestedProperties) && resourceDefinition.equals(query.resourceDefinition)
				&& keyValueMap.equals(query.keyValueMap) && !(userPredicate != null ? !userPredicate.equals(query.userPredicate) : query.userPredicate != null);
	}
	...
	/**
	 * Query the cluster controller for the sub-resources associated with this query object.
	 */
	 ...
	 */
	private Set<Resource> doQuery(Resource.Type type, Request request, Predicate predicate) throws UnsupportedPropertyException, SystemException, NoSuchResourceException, NoSuchParentResourceException {
	...
	/**
	 * Get a map of property sets keyed by the resources associated with this query. The property sets should contain the joined sets of all of the requested properties from each resource's sub-resources. For example, if this query is associated with the resources AResource1,
	 * AResource1 and AResource3 as follows ... a_resources │ └──AResource1 ─────────────AResource1 ─────────────AResource3 │ │ │ ├── b_resources ├── b_resources ├── BResources │ ├── BResource1 │ ├── BResource3 │ └── BResource5 │ │ p1:1 │ │ p1:3 │ p1:5 │ │ p2:5 │ │ p2:5 │ p2:5 │
	 * │ │ │ │ │ └── BResource2 │ └── BResource4 └── c_resources │ p1:2 │ p1:4 └── CResource4 │ p2:0 │ p2:0 p3:4 │ │ └── c_resources └── c_resources ├── CResource1 └── CResource3 │ p3:1 p3:3 │ └── CResource2 p3:2 Given the following query ...
	 * api/v1/a_resources?b_resources/p1>3&b_resources/p2=5&c_resources/p3=1 The caller should pass the following property ids ... b_resources/p1 b_resources/p2 c_resources/p3 getJoinedResourceProperties should produce the following map of property sets by making recursive calls
	 * on the sub-resources of each of this query's resources, joining the resulting property sets, and adding them to the map keyed by the resource ... { AResource1=[{b_resources/p1=1, b_resources/p2=5, c_resources/p3=1}, {b_resources/p1=2, b_resources/p2=0, c_resources/p3=1},
	 * {b_resources/p1=1, b_resources/p2=5, c_resources/p3=2}, {b_resources/p1=2, b_resources/p2=0, c_resources/p3=2}], AResource2=[{b_resources/p1=3, b_resources/p2=5, c_resources/p3=3}, {b_resources/p1=4, b_resources/p2=0, c_resources/p3=3}], AResource3=[{b_resources/p1=5,
	 * b_resources/p2=5, c_resources/p3=4}], }
	 * 
	...
	protected Map<Resource, Set<Map<String, Object>>> getJoinedResourceProperties(Set<String> propertyIds, Resource parentResource, String category) throws SystemException, UnsupportedPropertyException, NoSuchParentResourceException, NoSuchResourceException {
		...
		for (Map.Entry<Resource, QueryResult> queryResultEntry : queryResults.entrySet()) {
			...
			if (queryParentResource == parentResource) {
				...
				Iterable<Resource> iterResource = clusterController.getIterable(resourceDefinition.getType(), queryResult.getProviderResourceSet(), queryResult.getRequest(), queryResult.getPredicate());
	...
	/**
	 * Merge the tree of query properties returned by the renderer with properties in the query tree.
	 * 
	...
	private static Set<Map<String, Object>> joinPropertyMaps(Set<Map<String, Object>> propertyMaps1, Set<Map<String, Object>> propertyMaps2) {
		...
		if (propertyMaps1.isEmpty()) { return propertyMaps2; }
		if (propertyMaps2.isEmpty()) { return propertyMaps1; }
	...
	private Result getResult(Resource parentResource) throws UnsupportedPropertyException, SystemException, NoSuchResourceException, NoSuchParentResourceException {
		...
		if (queryResult != null) {
			...
			if (pageRequest == null) {
				iterResource = clusterController.getIterable(resourceType, providerResourceSet, queryRequest, queryPredicate);
			} else {
				PageResponse pageResponse = clusterController.getPage(resourceType, providerResourceSet, queryRequest, queryPredicate, pageRequest);
	...
	private Predicate createPredicate(Map<Resource.Type, String> keyValueMap, Predicate predicate) {
		...
		if (internalPredicate == null) { return predicate; }
	...
	private Request createRequest() {
		...
		if (allProperties) { return PropertyHelper.getReadRequest(Collections.<String> emptySet(), requestInfoProperties, null); }
	 ...
	 * @return either the plural or singular sub-resource name based on whether the sub-resource is included as a collection
	 */
	...
	private static class QueryResult {
		...
		private QueryResult(Request request, Predicate predicate, Predicate userPredicate, Map<Resource.Type, String> keyValueMap, Set<Resource> providerResourceSet) {
