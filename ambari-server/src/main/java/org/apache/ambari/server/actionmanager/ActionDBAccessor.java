...
public interface ActionDBAccessor {
	...
	/**
	 * Returns all the pending stages, including queued and not-queued. A stage is considered in progress if it is in progress for any host.
	 */
	...
	/**
	 * Return the last persisted Request ID as seen when the DBAccessor object was initialized. Value should remain unchanged through the lifetime of the object instance.
	 * 
	...
	/**
	 * Get a list of host role commands where the request id belongs to the input requestIds and the task id belongs to the input taskIds
	 */
	 ...
	 * 
	 * @param status Desired request status
	 * @param maxResults maximal number of returned id's
	 * @param ascOrder defines sorting order for database query result
	 * @return First or last maxResults request id's if ascOrder is true or false, respectively
	 */
