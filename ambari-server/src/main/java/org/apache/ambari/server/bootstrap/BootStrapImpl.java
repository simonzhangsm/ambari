...
public class BootStrapImpl {
	 ...
	 * @return status for a specific response id. A response Id of -1 means the latest responseId.
	 */
	public synchronized BootStrapStatus getStatus(long requestId) {
		if (!bsStatus.containsKey(Long.valueOf(requestId))) { return null; }
	...
	public synchronized void init() throws IOException {
		if (!bootStrapDir.exists()) {
			...
			if (!mkdirs) throw new IOException("Unable to make directory for " + "bootstrap " + bootStrapDir);
		}
	}
	...
	public synchronized BSResponse runBootStrap(SshHostInfo info) {
		...
		bsRunner = new BSRunner(this, info, bootStrapDir.toString(), bootScript, bootSetupAgentScript, bootSetupAgentPassword, requestId, 0L, this.masterHostname, info.isVerbose(), this.clusterOsType, this.projectVersion, this.serverPort);
		bsRunner.start();
