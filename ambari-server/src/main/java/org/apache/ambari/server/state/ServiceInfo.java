...
public class ServiceInfo {
	...
	/**
	 * Map of of os-specific details that is exposed (and initialised from list) at getter. Added at schema ver 2
	 */
	...
	/**
	 * Meaning: stores subpath from stack root to exact directory, that contains service scripts and templates. Since schema ver 2, we may have multiple service metadata inside folder. Added at schema ver 2
	 */
	@XmlTransient
	...
	public boolean isClientOnlyService() {
		...
		for (ComponentInfo compInfo : components) {
			if (!compInfo.isClient()) { return false; }
		}
		return true;
	}
	...
	public ComponentInfo getClientComponent() {
		...
		for (ComponentInfo compInfo : components) {
			if (compInfo.isClient()) { return compInfo; }
		}
	...
	public StackServiceResponse convertToResponse() {
		return new StackServiceResponse(getName(), getUser(), getComment(), getVersion(), getConfigTypes());
	}
	
	public List<String> getConfigTypes() {
		return configDependencies != null ? configDependencies : Collections.unmodifiableList(new ArrayList<String>());
	}
	...
	/**
	 * The purpose of this method is to determine if a service has a property defined in a supplied set:
	 * <ul>
	...
	public boolean hasPropertyFor(String type, Collection<String> keyNames) {
		if (!hasConfigType(type)) return false;
