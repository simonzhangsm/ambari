...
public class PredicateBuilder {
	...
	private void checkDone() {
		if (done) { throw new IllegalStateException("Can't reuse a predicate builder."); }
	}
	...
	private void handleComparator() {
		...
		if (predicates.size() == 0) { throw new IllegalStateException("No left operand."); }
		Predicate predicate;
	...
	public class PredicateBuilderWithProperty {
		...
		public <T> PredicateBuilderWithPredicate equals(Comparable<T> value) {
			if (propertyId == null) { throw new IllegalStateException("No property."); }
	...
	public class PredicateBuilderWithPredicate {
		...
		public Predicate toPredicate() {
			if (outer != null) { throw new IllegalStateException("Unbalanced block - missing end."); }
			done = true;
			return getPredicate();
		}
		
		public PredicateBuilderWithPredicate end() {
			if (outer == null) { throw new IllegalStateException("Unbalanced block - missing begin."); }
