...
public class ComponentInfo {
	...
	/**
	 * Auto-deployment information. If auto-deployment is enabled and the component doesn't meet the cardinality requirement, the component is auto-deployed to the cluster topology.
	 */
	...
	public StackServiceComponentResponse convertToResponse() {
		StackServiceComponentResponse response = new StackServiceComponentResponse(getName(), getCategory(), isClient(), isMaster(), cardinality);
	...
	public boolean isCustomCommand(String commandName) {
		if (customCommands != null && commandName != null) {
			for (CustomCommandDefinition cc : customCommands) {
				if (commandName.equals(cc.getName())) { return true; }
			}
		}
		return false;
	}
