...
public interface Host {
	...
	/**
	 * Get the Amount of available memory for the Host. In most cases, available should be same as total unless the agent on the host is configured to not use all available memory
	 * 
	...
	/**
	 * Get the OS Architecture. i386, x86_64, etc.
	 * 
	 * @return the osArch
	 */
	...
	/**
	 * Get the General OS information. uname -a, /etc/*-release dump
	 * 
	 * @return the osInfo
	 */
	...
	/**
	 * Get the OS Type: RHEL5/RHEL6/CentOS5/... Defined and match-able OS type
	 * 
	 * @return the osType
	 */
	...
	/**
	 * Get the os Family: redhat5: for centos5, rhel5, oraclelinux5 .. redhat6: for centos6, rhel6, oraclelinux6 .. debian12 : for ubuntu12 suse11: for sles11, suse11 ..
	 * 
	 * @return the osFamily
	 */
	 ...
	 * @param selected <code>true</code> if the configuration is selected. Applies only to remove the override, otherwise this value should always be <code>true</code>.
	 * @param user the user making the change for audit purposes
	 * @param config the configuration object
	 * @return <code>true</code> if the config was added, or <code>false</code> if the config is already set as the current
	 */
	public boolean addDesiredConfig(long clusterId, boolean selected, String user, Config config);
	
	/**
	 * Gets all the selected configurations for the host. return a map of type-to-{@link DesiredConfig} instances.
	 */
