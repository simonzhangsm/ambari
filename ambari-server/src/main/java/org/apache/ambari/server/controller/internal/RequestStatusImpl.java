...
public class RequestStatusImpl implements RequestStatus {
	...
	public RequestStatusImpl(Resource requestResource, Set<Resource> associatedResources) {
		this.requestResource = requestResource;
		this.associatedResources = associatedResources == null ? Collections.<Resource> emptySet() : associatedResources;
	}
	...
	public Status getStatus() {
		...
		return requestResource == null ? Status.Complete : Status.valueOf((String) requestResource.getPropertyValue(PropertyHelper.getPropertyId("Requests", "status")));
	}
}
