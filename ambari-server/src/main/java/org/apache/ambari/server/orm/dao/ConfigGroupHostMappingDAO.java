...
public class ConfigGroupHostMappingDAO {
	 ...
	 */
	@RequiresSession
	public ConfigGroupHostMappingEntity findByPK(final ConfigGroupHostMappingEntityPK configGroupHostMappingEntityPK) {
		
		return entityManagerProvider.get().find(ConfigGroupHostMappingEntity.class, configGroupHostMappingEntityPK);
	}
	...
	public Set<ConfigGroupHostMapping> findByHost(String hostname) {
		...
		if (!configGroupHostMappingByHost.containsKey(hostname)) return null;
	...
	public void removeAllByHost(String hostname) {
		TypedQuery<String> query = entityManagerProvider.get().createQuery("DELETE FROM ConfigGroupHostMappingEntity confighosts WHERE " + "confighosts.hostname = ?1", String.class);
