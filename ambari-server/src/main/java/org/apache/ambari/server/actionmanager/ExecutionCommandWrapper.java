...
public class ExecutionCommandWrapper {
		...
		if (executionCommand != null) {
			...
			if (injector == null) {
				...
				try {
					...
					Map<String, Map<String, String>> configProperties = configHelper.getEffectiveConfigProperties(cluster, executionCommand.getConfigurationTags());
	...
	public int hashCode() {
		...
		} else if (jsonExecutionCommand != null) { return jsonExecutionCommand.hashCode(); }
		throw new RuntimeException("Invalid Wrapper object");
	}
	
	void invalidateJson() {
		if (executionCommand == null) { throw new RuntimeException("Invalid Wrapper object"); }
