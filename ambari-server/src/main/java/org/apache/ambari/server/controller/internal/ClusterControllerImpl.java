...
public class ClusterControllerImpl implements ClusterController {
	private final static Logger LOG = LoggerFactory.getLogger(ClusterControllerImpl.class);
	...
	@Override
	public Iterable<Resource> getIterable(Resource.Type type, Set<Resource> providerResources, Request request, Predicate predicate) throws NoSuchParentResourceException, UnsupportedPropertyException, NoSuchResourceException, SystemException {
		return getPage(type, providerResources, request, predicate, null).getIterable();
	}
	
	@Override
	public PageResponse getPage(Resource.Type type, Set<Resource> providerResources, Request request, Predicate predicate, PageRequest pageRequest) throws UnsupportedPropertyException, SystemException, NoSuchResourceException, NoSuchParentResourceException {
	...
	@Override
	public RequestStatus deleteResources(Resource.Type type, Predicate predicate) throws UnsupportedPropertyException, SystemException, NoSuchResourceException, NoSuchParentResourceException {
	 ...
	 * @throws UnsupportedPropertyException thrown if the request or predicate contain unsupported property ids
	 ...
	 * @throws NoSuchParentResourceException a specified parent resource doesn't exist
	 */
	protected PageResponse getResources(Resource.Type type, Request request, Predicate predicate, PageRequest pageRequest) throws UnsupportedPropertyException, SystemException, NoSuchResourceException, NoSuchParentResourceException {
	...
	/**
	 * Check to make sure that all the property ids specified in the given request and predicate are supported by the resource provider or property providers for the given type.
	 * 
	 * @param type the resource type
	 * @param request the request
	 * @param predicate the predicate
	 * @return true if all of the properties specified in the request and predicate are supported by the resource provider for the given type; false if any of the properties specified in the request and predicate are not supported by the resource provider but are supported by a
	 *         property provider for the given type.
	 * @throws UnsupportedPropertyException thrown if any of the properties specified in the request and predicate are not supported by either the resource provider or a property provider for the given type
	 */
		...
		if (requestPropertyIds.size() > 0) {
			...
			if (requestPropertyIds.size() > 0) {
				...
				for (PropertyProvider propertyProvider : propertyProviders) {
					...
					if (requestPropertyIds.size() == 0) { return false; }
				}
	...
	/**
	 * Check to see if any of the property ids specified in the given request and predicate are handled by an associated property provider. if so, then use the given predicate to obtain a new predicate that can be completely processed by an update or delete operation on a
	 * resource provider for the given resource type. This means that the new predicate should only reference the key property ids for this type.
	 * 
	 * @param type the resource type
	 * @param predicate the predicate
	...
	private boolean providesRequestProperties(PropertyProvider provider, Request request, Predicate predicate) {
		...
		if (requestPropertyIds.size() == 0) { return true; }
		requestPropertyIds.addAll(PredicateHelper.getPropertyIds(predicate));
	 ...
	 */
	private PageResponse getPageFromOffset(int pageSize, int offset, NavigableSet<Resource> resources, Predicate predicate, ResourcePredicateEvaluator evaluator) {
		...
		return new PageResponseImpl(new ResourceIterable(pageResources, predicate, evaluator), currentOffset, previous, iterator.hasNext() ? iterator.next() : null);
	}
	 ...
	 * @return a page response containing a page of resources
	 */
	private PageResponse getPageToOffset(int pageSize, int offset, NavigableSet<Resource> resources, Predicate predicate, ResourcePredicateEvaluator evaluator) {
		...
		return new PageResponseImpl(new ResourceIterable(new LinkedHashSet<Resource>(pageResources), predicate, evaluator), currentOffset + 1, iterator.hasNext() ? iterator.next() : null, next);
	}
	...
	private static class ResourceIterator implements Iterator<Resource> {
		...
		public Resource next() {
			if (nextResource == null) { throw new NoSuchElementException("Iterator has no more elements."); }
		...
		private Resource getNextResource() {
			while (iterator.hasNext()) {
				...
				if (predicate == null || evaluator.evaluate(predicate, next)) { return next; }
			}
			return null;
		}
	}
	...
	protected class ResourceComparator implements Comparator<Resource> {
		...
		private int compareValues(Object val1, Object val2) {
			...
			if (val1 == null || val2 == null) { return val1 == null && val2 == null ? 0 : val1 == null ? -1 : 1; }
