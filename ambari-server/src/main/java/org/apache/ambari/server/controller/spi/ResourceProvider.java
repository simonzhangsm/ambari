...
/**
 * The resource provider allows for the plugging in of a back end data store for a resource type. The resource provider is associated with a specific resource type and can be queried for a list of resources of that type. The resource provider plugs into and is used by the
 * {@link ClusterController cluster controller} to obtain a list of resources for a given request.
 */
public interface ResourceProvider {
	...
	/**
	 * Get a set of {@link Resource resources} based on the given request and predicate information. </p> Note that it is not required for this resource provider to completely filter the set of resources based on the given predicate. It may not be possible since some of the
	 * properties involved may be provided by another {@link PropertyProvider provider}. This partial filtering is allowed because the predicate will always be applied by the calling cluster controller. The predicate is made available at this level so that some pre-filtering can
	 * be done as an optimization. </p> A simple implementation of a resource provider may choose to just return all of the resources of a given type and allow the calling cluster controller to filter based on the predicate.
	 * 
	...
	/**
	 * Get the key property ids for the resource type associated with this resource provider. The key properties are those that uniquely identify the resource. </p> For example, the resource 'HostComponent' is uniquely identified by its associated 'Cluster', 'Host' and
	 * 'Component' resources. The key property ids for a 'HostComponent' resource includes the property ids of the foreign key references from the 'HostComponent' to 'Cluster', 'Host' and 'Component' resources.
	 * 
	 ...
	 * 
	 * @return a subset of the given property id set containing any property ids not supported by this resource provider. An empty return set indicates that all of the given property ids are supported.
	 */
