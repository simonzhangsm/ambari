...
public class RequestDAO {
	...
	public List<RequestEntity> findByPks(Collection<Long> requestIds) {
		if (null == requestIds || 0 == requestIds.size()) return Collections.emptyList();
	...
	public boolean isAllTasksCompleted(long requestId) {
		TypedQuery<Long> query = entityManagerProvider.get().createQuery(
				"SELECT task.taskId FROM HostRoleCommandEntity task WHERE task.requestId = ?1 AND " + "task.stageId=(select max(stage.stageId) FROM StageEntity stage WHERE stage.requestId=?1) " + "AND task.status NOT IN ?2", Long.class);
