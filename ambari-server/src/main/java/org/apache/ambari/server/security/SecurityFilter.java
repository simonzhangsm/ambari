...
public class SecurityFilter implements Filter {
	...
	private boolean isRequestAllowed(String reqUrl) {
		try {
			...
			if (Pattern.matches("/cert/ca(/?)", url.getPath())) { return true; }
			
			if (Pattern.matches("/certs/[^/0-9][^/]*", url.getPath())) { return true; }
			
			if (Pattern.matches("/resources/.*", url.getPath())) { return true; }
